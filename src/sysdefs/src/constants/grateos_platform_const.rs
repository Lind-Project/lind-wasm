//! This file defines constants that are specific to the GrateOS-Wasm platform.
//!
/// ===== GrateOS File System Root =====
///
/// Maximum allowed path length in GrateOS.  
...
/// Maximum linear memory size for a single Wasm module in the current grateos-wasm runtime.
/// Since grateos-wasm uses 32-bit memories, the linear memory address space is limited to 4 GiB.
...
///
/// **This limit may be adjusted in the future if grateos-wasm adopts 64-bit memories
...
///
/// ## Usage scenarios
/// 1. During `grateos-boot` initialization, syscalls that are expected to
...
///
/// ## Usage scenarios
/// - Used during `grateos-boot` initialization when registering Wasmtime
...
///   during `grateos-boot` initialization in RawPOSIX.
...
/// The starting index for function tables of wasm modules in GrateOS.
...
///
/// This is a platform-wide configuration constant. grateos-wasm preallocates
...
/// rather than being owned by `grateos-3i`, `grateos-multi-process`, or Wasmtime
