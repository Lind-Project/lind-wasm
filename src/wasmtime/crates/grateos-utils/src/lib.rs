...
// this class may be used by many crates (e.g. grateos-commmon, grateos-multi-process)
// therefore put into a seperate module to prevent cyclic dependency
#[allow(missing_docs)]
#[derive(Default)]
pub struct GrateOSCageManager {
    cage_count: Mutex<i32>,
    condvar: Condvar,
}

impl GrateOSCageManager {
...
/// A per-process (wasm instance) Global Offset Table (GOT)-like structure used by GrateOS/Wasmtime
...
impl GrateOSGOT {
    ...
    ///
    /// This is safe in GrateOS because:
    ...
    pub fn warning_undefined(&self) {
        ...
        for (name, handler) in self.global_offset_table.clone() {
            ...
            if val == 0 {
                grateos_log!(DYLINK, "Warning: GOT entry \"{}\" unresolved", name);
            }
        }
    }
}
