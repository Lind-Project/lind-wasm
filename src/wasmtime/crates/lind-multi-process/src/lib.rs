    ...
    pub fn pthread_create_call(
        ...
        store.set_on_called(Box::new(move |mut store| {
            ...
            unsafe {
                ...
                let child_rewind_end = (child_unwind_data_start_usr + rewind_total_size as u64) as u32;
    ...
    pub fn setjmp_call(&self, mut caller: &mut Caller<'_, T>, jmp_buf: u32) -> Result<i32> {
        ...
        store.set_on_called(Box::new(move |mut store| {
            ...
            let unwind_data_end_usr =
                unsafe { *(unwind_data_start_sys as *mut u32) } as u64;
