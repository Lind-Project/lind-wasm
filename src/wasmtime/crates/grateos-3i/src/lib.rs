//! This module provides a global runtime-state lookup mechanism for grateos-3i and grateos-wasm, enabling
//! controlled transfers of execution across cages, grates, and threads.
//!
//! In grateos-wasm, runtime control is not always confined to a single Wasmtime instance or a single
//! linear call stack. There are two primary scenarios in which grateos-3i must explicitly locate and
...
//! a result, grateos-3i cannot rely on an implicit “current” runtime state. Instead, it must be able to
...
//! return values.
//!
//! As a result, grateos-3i must be able to retrieve *the active execution context* corresponding
...
//! only needs to enter a compatible execution context for the target grate. In grateos-3i, this compatible
...
//! worker its own `Store` and `Instance`, grateos-3i ensures that concurrent grate calls do not execute
...
//! operations. For fork / exit, grateos-3i must recover the specific active execution context associated with
//! a given (cage_id, tid), because execution must resume in the same continuation. For grate calls, by
//! contrast, grateos-3i only needs to obtain some available worker for the target grate, because correctness
...
const GRATE_WORKERS_ENV: &str = "GRATEOS_GRATE_WORKERS";
...
fn worker_stack_base(cageid: u64, workerid: WorkerId) -> u32 {
    let stack_arena_base = grateos_platform_const::get_stack_arena_base(cageid as usize)
        .unwrap_or_else(|| {
...
fn configured_grate_workers() -> usize {
    // Defaults to MAX_GRATE_WORKERS; set GRATEOS_GRATE_WORKERS to configure a smaller positive pool size.
...
///
/// To preserve isolation between concurrent grate calls, grateos-wasm partitions
...
/// `instantiate_with_grateos_thread()`. See that function’s comments for the
    ...
    T: Clone + 'static,
{
    ...
    let (instance, _, _) = linker
        .instantiate_with_grateos_thread(&mut store, &template.module, false)
...
///
/// By default, the pool size is MAX_GRATE_WORKERS; set GRATEOS_GRATE_WORKERS
/// to configure it.
...
///
/// This function must be called exactly once during grateos-wasm startup, before any `VMContext` is
