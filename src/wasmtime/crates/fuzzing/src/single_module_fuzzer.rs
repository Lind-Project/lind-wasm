...
mod tests {
    ...
    fn changing_configuration_does_not_change_module() {
        ...
        for _ in 0..200 {
            ...
            if let Ok((module, known_valid)) = execute(&buf[..seed_size], run1, gen_fn) {
                assert_eq!(known_valid, KnownValid::Yes);
                let new_size = mutate(&mut buf, seed_size, max_size, gen_fn, noop_mutate);
                if let Ok((module2, known_valid)) = execute(&buf[..new_size], run2, gen_fn) {
        ...
        fn gen_fn<T>(_: &mut T, u: &mut Unstructured<'_>) -> Result<(Vec<u8>, KnownValid)>
        where
            T: for<'a> Arbitrary<'a>,
        {
            Ok((
