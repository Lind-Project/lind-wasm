...
use wasmtime_grateos_multi_process::{GrateOSHost, get_memory_base, get_memory_base_and_size};
// These syscalls (`clone`, `exec`, `exit`, `fork`) require special handling
// inside GrateOS Wasmtime before delegating to RawPOSIX. For example, they may
...
pub struct GrateOSEnviron {
    args: Vec<String>,
    env: Vec<(String, String)>,
}

impl GrateOSEnviron {
    /// Build from program arguments and `--env` flags passed on the grateos-boot
...
>(
    ...
    linker.func_wrap(
        ...
        move |mut caller: Caller<'_, T>,
            ...
            //
            // `clone` is particularly sensitive here: during a logical `clone`, the grateos
            ...
            //
            // In grateos-boot we forward syscalls directly to RawPOSIX, so we replicate the state
            // check here to early-return when we are on a rewind replay path.
            if call_number as i32 == CLONE_SYSCALL {
                if let Some(rewind_res) = wasmtime_grateos_multi_process::catch_rewind(&mut caller) {
                    if rewind_res > 0 {
                        ...
                        if let Some(retval) = wasmtime_grateos_multi_process::take_pending_clone_visible_retval(&mut caller) {
            ...
            if let AsyncifyState::Rewind(_) = caller.as_context().get_asyncify_state() {
                let retval = match caller.as_context_mut().get_current_syscall_rewind_data() {
                    Some(v) => v,
                    None => {
                        wasmtime_grateos_multi_process::signal::signal_handler(&mut caller)?;
                        return Ok(0);
                    }
                };
            ...
            let final_arg2 = if target_cageid == self_cageid
                ...
                wasmtime_grateos_multi_process::current_tid(&mut caller) as u64
            } else {
                arg2
            };
            ...
            if call_number as i32 == CLONE_SYSCALL {
                ...
                wasmtime_grateos_multi_process::set_pending_clone_visible_retval(&mut caller, retval).unwrap_or_else(|e| {
                    panic!("{}", format!(
...
>(
    ...
    linker.func_wrap(
        ...
        move |mut caller: Caller<'_, T>| -> u64 {
            let cageid = wasmtime_grateos_multi_process::current_cageid(&mut caller) as u64;
            cageid
        },
    )?;

    linker.func_wrap("grateos", "debug-panic", move |str: u64| -> () {
        let _panic_str = unsafe { std::ffi::CStr::from_ptr(str as *const i8).to_str().unwrap() };
        grateos_debug_panic!("FROM GUEST: {}", _panic_str);
    })?;
    ...
    linker.func_wrap(
        ...
        move |mut caller: Caller<'_, T>, jmp_buf: i32| -> i32 {
            wasmtime_grateos_multi_process::setjmp_call(&mut caller, jmp_buf as u32)
        },
    )?;
    ...
    linker.func_wrap(
        ...
        move |mut caller: Caller<'_, T>, jmp_buf: i32, retval: i32| -> i32 {
            wasmtime_grateos_multi_process::longjmp_call(&mut caller, jmp_buf as u32, retval)
        },
    )?;
...
>(
    ...
    linker.func_wrap(
        ...
        move |_caller: Caller<'_, T>, num: u32| -> u32 {
            eprintln!("[GRATEOS DEBUG NUM]: {}", num);
            num
        },
    )?;
    ...
    linker.func_wrap(
        ...
        move |mut caller: Caller<'_, T>, ptr: i32| -> i32 {
            ...
            if let Ok(msg) = get_cstr(mem_base + (ptr as u32) as u64) {
                eprintln!("[GRATEOS DEBUG STR]: {}", msg);
            }
            ptr
        },
    )?;
...
///
/// glibc's `_start()` imports these from `"grateos"`, while Rust std compiled with
...
>(
    linker: &mut wasmtime::Linker<T>,
    module: &str,
    get_environ: impl Fn(&T) -> &GrateOSEnviron + Send + Sync + Copy + 'static,
) -> anyhow::Result<()> {
    linker.func_wrap(
        module,
        "args_sizes_get",
...
>(
    ...
    linker.func_wrap(
        ...
        move |mut caller: wasmtime::Caller<'_, T>, file: i32, mode: i32| -> i32 {
            if dylink_enabled {
                wasmtime_grateos_dylink::dlopen_call(
                    &mut caller,
                    file,
                    mode,
                ...
                grateos_debug_panic!("dynamic loading support is not enabled!");
    ...
    linker.func_wrap(
        ...
        move |mut caller: wasmtime::Caller<'_, T>, handle: i32, name: i32| -> i32 {
            if dylink_enabled {
                wasmtime_grateos_dylink::dlsym_call(&mut caller, handle, name)
            } else {
                grateos_debug_panic!("dynamic loading support is not enabled!");
                -(sysdefs::constants::DylinkErrorCode::EOPEN as i32)
            }
    ...
    linker.func_wrap(
        ...
        move |mut caller: wasmtime::Caller<'_, T>, handle: i32| -> i32 {
            if dylink_enabled {
                wasmtime_grateos_dylink::dlclose_call(&mut caller, handle)
            } else {
                grateos_debug_panic!("dynamic loading support is not enabled!");
                -(sysdefs::constants::DylinkErrorCode::EOPEN as i32)
            }
...
/// Register all GrateOS host functions with the linker.
///
/// Groups:
...
/// - **debug** (grateos_debug feature only): `grateos_debug_num`, `grateos_debug_str`
/// - **environ**: argv/environ/random_get under both `"grateos"` and `"wasi_snapshot_preview1"`
...
>(
    ...
    add_environ_funcs_to_linker(linker, "grateos", get_environ)?;
