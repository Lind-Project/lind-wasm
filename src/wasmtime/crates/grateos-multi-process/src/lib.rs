...
use cage::signal::{grateos_signal_init, grateos_thread_exit};
...
pub trait GrateOSHost<T, U> {
    fn get_ctx(&self) -> GrateOSCtx<T, U>;
    fn get_ctx_mut(&mut self) -> &mut GrateOSCtx<T, U>;
}
...
pub struct GrateOSCtx<T, U> {
    ...
    // from grateos-boot, used for exec call
    grateosboot_cli: U,

    // Optional parent-visible return value for a pending `clone`/`fork`.
    //
    // `fork` is special in the GrateOS/Wasmtime runtime because the guest-visible
    ...
    // host/GrateOS context can be cloned during fork/rewind setup. Using an
    ...
    // get GrateOSCtx from host
    get_cx: Arc<dyn Fn(&mut T) -> &mut GrateOSCtx<T, U> + Send + Sync + 'static>,
    ...
    GrateOSCtx<T, U>
{
    ...
    // * grateos_manager: global grateos cage counter. Used to make sure the wasmtime runtime would only exit after all cages have exited
    // * grateosboot_cli: used by exec closure below.
    // * cageid: cageid associated with the context
    // * get_cx: get grateosContext from Host object
    ...
    pub fn new(
        ...
        get_cx: impl Fn(&mut T) -> &mut GrateOSCtx<T, U> + Send + Sync + 'static,
        ...
        // this method should only be called once from run.rs, other instances of GrateOSCtx
        ...
        Ok(Self {
            ...
            tid,
            next_threadid,
            grateos_manager: grateos_manager.clone(),
            grateosboot_cli,
    ...
    // Attach a GrateOSGOT (Global Offset Table) to this context, wrapping it in
    ...
    pub fn attach_got_table(&mut self, got_table: Option<GrateOSGOT>) {
    ...
    pub fn fork_call(&self, mut caller: &mut Caller<'_, T>, child_cageid: u64) -> Result<i32> {
        ...
        store.set_on_called(Box::new(move |mut store| {
            ...
            let builder = thread::Builder::new()
                .name(format!("grateos-fork-{}", child_cageid))
            ...
            builder
                .spawn(move || {
                    ...
                    let grateos_manager = child_ctx.grateos_manager.clone();
                    ...
                    let child_table = if dylink_enabled {
                        ...
                        for (name, path, module) in modules.iter().skip(1) {
                            ...
                            let module_name = module.name().unwrap_or_else(|| {
                                grateos_debug_panic!("module has no name");
                    ...
                    let epoch_pointer = if epoch_handler.is_some() {
                        ...
                        cfg_if! {
                            ...
                            if #[cfg(feature = "disable_signals")] {
                                ...
                                grateos_epoch.get_handler_as_u64(&mut store)
                            }
                        }
                    };
                    ...
                    if store.is_thread() {
                        ...
                        if let Err(err) = invoke_res {
                            grateos_log!(Default, "Child Error: {:?}", err);
                            ...
                            cage::signal::grateos_thread_exit(child_cageid, THREAD_START_ID as u64);
                        ...
                        match exit_code {
                            Val::I32(val) => {}
                            _ => {
                                grateos_log!(Default, "unexpected _start function return type!");
                            }
                        }
                    }
    ...
    pub fn pthread_create_call(
        ...
        let next_tid = match self.next_thread_id() {
            Some(val) => val,
            None => {
                grateos_log!(Default, "running out of thread id!");
                0
            }
        };
        ...
        store.set_on_called(Box::new(move |mut store| {
            ...
            let builder = thread::Builder::new()
                .name(format!("grateos-thread-{}", next_tid))
                .stack_size(thread_stack_size);
            builder
                .spawn(move || {
                    // create a new instance
                    ...
                    let (instance, _, grate_instanceid) = linker
                        .instantiate_with_grateos_thread(&mut store, &module, false)
                        .unwrap();
    ...
    pub fn execve_call(
        ...
        let cloned_grateosboot_cli = self.grateosboot_cli.clone();
        let cloned_grateos_manager = self.grateos_manager.clone();
        ...
        store.set_on_called(Box::new(move |mut store| {
            ...
            let ret = exec_call(
                &cloned_grateosboot_cli,
                &path,
                &argv,
                engine,
                exec_module,
                cloned_cageid,
                &cloned_grateos_manager,
                &environs,
            );
            ...
            if let Err(ref _e) = ret {
                ...
                cage::signal::grateos_thread_exit(cloned_cageid as u64, THREAD_START_ID as u64);
                cage::cage_finalize(cloned_cageid as u64);
                cloned_grateos_manager.decrement();
    ...
    pub fn exit_call(&self, mut caller: &mut Caller<'_, T>, code: i32, _is_last_thread: u64) {
        ...
        // Every thread defers grateos_thread_exit to OnCalledAction so that
        ...
        let deferred_grateos_manager = self.grateos_manager.clone();
        // get the base address of the memory
        let address = get_memory_base(&mut caller) as *mut u8;
        ...
        store.set_on_called(Box::new(move |mut store| {
            ...
            // actual last thread in the cage, grateos_thread_exit returns
            // true and we handle full cage teardown.
            let is_last = cage::signal::grateos_thread_exit(deferred_cageid, deferred_tid);
            if is_last {
                ...
                deferred_grateos_manager.decrement();
            }
    ...
    // return true if the grateos process had thread before
    ...
    pub fn fork_process(&self) -> Self {
        ...
        let forked_ctx = Self {
            ...
            cageid: 0,                                  // cageid is managed by grateos-common
            tid: 1,                                     // thread id starts from 1
            next_threadid: Arc::new(AtomicU32::new(1)), // thread id starts from 1
            grateos_manager: self.grateos_manager.clone(),
            grateosboot_cli: self.grateosboot_cli.clone(),
...
pub fn grateos_fork<
    T: GrateOSHost<T, U> + Clone + Send + 'static + std::marker::Sync,
...
///
/// This function is the **Wasmtime re-entry trampoline** used by the GrateOS to
...
///         -> grateos_fork / grateos_pthread_create
///
/// During `grateos-boot` initialization, this function is extracted as a raw `u64`
...
/// per-thread VMContext table. See more details on [grateos-3i/src/lib.rs].
...
where
    T: GrateOSHost<T, U> + Clone + Send + Sync + 'static,
    U: Clone + Send + Sync + 'static,
{
    unsafe {
        ...
        let ret = Caller::with(vmctx_raw, |mut caller: Caller<'_, T>| {
            ...
            if isthread == 0 {
                // fork
                match grateos_fork(&mut caller, child_cageid) {
                    Ok(res) => res,
                    Err(_e) => -1,
                }
            } else {
                // pthread_create
                match grateos_pthread_create(
                    &mut caller,
                    args.stack as u32,
...
/// table during grateos-boot initialization (see `grateos-boot/execute.rs` for the
...
/// bridging execution back from the GrateOS / RawPOSIX world into Wasmtime.
///
/// Conceptually, the execution flow is:
///   Wasm
///     -> Wasmtime grateos-common trampoline
...
///     -> **back to Wasmtime**
///
/// During `grateos-boot` initialization, this function is extracted as a raw
...
>(
    ...
    // In grateos-wasm the linear memory is always a fixed 4 GiB physical
    ...
    const GRATEOS_MAX_PAGES: u64 = 65536;

    let mem_type = wasmtime::MemoryTypeBuilder::new()
        .shared(true)
        .min(combined_min)
        .max(Some(GRATEOS_MAX_PAGES))
        .build()
    ...
    let delta = GRATEOS_MAX_PAGES.saturating_sub(combined_min);
    if delta > 0 {
        mem.grow(delta)
            .map_err(anyhow::Error::from)
    ...
    if need_init {
        ...
        // grateos-wasm: reset the entire 4 GiB wasm linear memory to PROT_NONE
        ...
        // new_started_impl_with_grateos re-establish the required initial regions
...
/// survives `HostCtx`/`GrateOSCtx` cloning across fork setup.
