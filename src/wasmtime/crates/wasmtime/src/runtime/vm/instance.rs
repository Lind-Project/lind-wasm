...
impl Instance {
    ...
    /// # Safety
    ///
    /// `vmctx` must be a valid pointer to a live VMContext with a valid store.
    pub(crate) unsafe fn store_and_id_from_vmctx(
        vmctx: NonNull<VMContext>,
    ) -> (*mut dyn VMStore, InstanceId) {
        unsafe {
            let instance = Instance::from_vmctx(vmctx);
            let instance = instance.as_ref();
            let store_ptr = instance.store.unwrap().0.as_ptr();
            (store_ptr, instance.id)
        }
    }
