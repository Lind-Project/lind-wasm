...
use sysdefs::{
    ...
    grateos_debug_panic, grateos_log,
};
use wasmtime::Caller;
use wasmtime_grateos_multi_process::{GrateOSHost, get_memory_base};
...
>(
    ...
    let val = if handle == RTLD_DEFAULT {
        ...
        grateos_debug_panic!("[grateos-dylink] dlsym RTLD_NEXT: not currently supported");
    ...
    grateos_log!(DYLINK, "dlsym resolves {} to {}", symbol, val);
    val as i32
}
