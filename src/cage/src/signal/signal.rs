...
pub fn signal_epoch_trigger(cageid: u64) {
    ...
    #[cfg(not(feature = "disable_signals"))]
    {
        let cage = match get_cage(cageid) {
            Some(c) => c,
            None => {
                grateos_log!("signal_epoch_trigger: cage {} not found", cageid);
...
pub fn signal_epoch_reset(cageid: u64) {
    ...
    #[cfg(not(feature = "disable_signals"))]
    {
        let cage = match get_cage(cageid) {
            Some(c) => c,
            None => {
                grateos_log!("signal_epoch_reset: cage {} not found", cageid);
...
pub fn signal_check_trigger(cageid: u64) -> bool {
    ...
    #[cfg(not(feature = "disable_signals"))]
    {
        let cage = match get_cage(cageid) {
            Some(c) => c,
            None => {
                grateos_log!("signal_check_trigger: cage {} not found", cageid);
...
pub fn signal_check_block(cageid: u64, signo: i32) -> bool {
    let cage = match get_cage(cageid) {
        Some(c) => c,
        None => {
            grateos_log!("signal_check_block: cage {} not found", cageid);
...
pub fn signal_get_handler(cageid: u64, signo: i32) -> u32 {
    let cage = match get_cage(cageid) {
        Some(c) => c,
        None => {
            grateos_log!("signal_get_handler: cage {} not found", cageid);
...
pub fn grateos_send_signal(cageid: u64, signo: i32) -> bool {
    debug_assert!(
        signo > 0 && signo < 32,
        "invalid signal number passed to grateos_send_signal"
    );
...
// thread safety: this function will only be invoked by main thread of the cage
pub fn grateos_get_first_signal(cageid: u64) -> Option<(i32, u32, Box<dyn Fn(u64)>)> {
...
// thread safety: this function will only be invoked by main thread of the cage
pub fn grateos_check_no_pending_signal(cageid: u64) -> bool {
    let cage = match get_cage(cageid) {
        Some(c) => c,
        None => {
            grateos_log!("grateos_check_no_pending_signal: cage {} not found", cageid);
...
// thread safety: this function could possibly be invoked by multiple threads of the same cage
pub fn grateos_signal_init(cageid: u64, epoch_handler: *mut u64, threadid: i32, is_mainthread: bool) {
    let cage = match get_cage(cageid) {
        Some(c) => c,
        None => {
            grateos_log!(
                "grateos_signal_init: cage {} not found for thread {}",
                cageid,
                threadid
            );
...
pub fn grateos_thread_exit(cageid: u64, thread_id: u64) -> bool {
    let cage = match get_cage(cageid) {
        Some(c) => c,
        None => {
            grateos_log!(
                "grateos_thread_exit: cage {} not found for thread {}",
                cageid,
                thread_id
            );
            return false;
            ...
            #[cfg(not(feature = "disable_signals"))]
            {
                ...
                if saved_epoch_state == EPOCH_SIGNAL {
                    ...
                    if new_thread_epoch_ptr.is_null() {
                        grateos_log!(
                            "grateos_thread_exit: epoch pointer for new main thread {} is null",
                            id
                        );
                        return false;
                    }
...
pub fn signal_may_trigger(cageid: u64) {
    let cage = match get_cage(cageid) {
        Some(c) => c,
        None => {
            grateos_log!("signal_may_trigger: cage {} not found", cageid);
