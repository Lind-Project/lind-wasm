...
pub extern "C" fn mmap_syscall(
    ...
    // Don't truncate grate-supplied host pointers (above u32 range): convert
    // back through the cage's vmmap base.  Cage-side uaddrs (≤ u32::MAX)
    // remain untouched.
    let mut useraddr = if addr_arg > u32::MAX as u64 {
        sc_convert_sys_to_user(addr_arg as usize, cageid).unwrap_or(0)
    } else {
        addr as u32
    };
