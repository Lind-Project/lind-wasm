...
pub fn get_specific_virtual_fd(
    ...
    // since the backing Vec below is sized FD_PER_PROCESS_MAX and indexed
    // 0..FD_PER_PROCESS_MAX). requested_virtualfd == FD_PER_PROCESS_MAX
    // previously passed this guard and indexed out of bounds, panicking
    // the host instead of returning EBADF (e.g. dup2(x, FD_PER_PROCESS_MAX)).
...
fn _increment_fdcount(entry: FDTableEntry) {
    ...
    // another cage still holds the fd. _decrement_fdcount uses this
    // entry() pattern already; this makes the two symmetric.
