...
use wasmtime_grateos_multi_process::{GrateOSCtx, GrateOSHost};
use wasmtime_grateos_utils::GrateOSGOT;

/// The HostCtx host structure stores all relevant execution context objects:
/// `grateos_environ`: argv/environ data served by the 4 host functions in grateos-common;
/// `grateos_fork_ctx`: the multi-process management structure, encapsulating fork/exec state;
#[derive(Default, Clone)]
pub struct HostCtx {
    pub grateos_environ: Option<Arc<GrateOSEnviron>>,
    pub grateos_fork_ctx: Option<GrateOSCtx<HostCtx, CliOptions>>,
}

impl HostCtx {
    /// Performs a partial deep clone of the host context. It explicitly forks the
    /// grateos_environ (argv/env) and the grateos multi-process context (`grateos_fork_ctx`).
    ...
    pub fn fork(&self) -> Self {
        let forked_grateos_environ = self.grateos_environ.as_ref().map(|e| Arc::new(e.fork()));

        let forked_grateos_fork_ctx = self.grateos_fork_ctx.as_ref().map(|ctx| ctx.fork_process());

        Self {
            grateos_environ: forked_grateos_environ,
            grateos_fork_ctx: forked_grateos_fork_ctx,
        }
    }

    pub fn fork_thread(&self) -> Self {
        let forked_grateos_fork_ctx = self.grateos_fork_ctx.as_ref().map(|ctx| ctx.fork_thread());

        Self {
            grateos_environ: self.grateos_environ.clone(), // clones the Arc, sharing the same GrateOSEnviron
            grateos_fork_ctx: forked_grateos_fork_ctx,
        }
    }
}

impl GrateOSHost<HostCtx, CliOptions> for HostCtx {
    fn get_ctx(&self) -> GrateOSCtx<HostCtx, CliOptions> {
        self.grateos_fork_ctx.clone().unwrap()
    }

    fn get_ctx_mut(&mut self) -> &mut GrateOSCtx<HostCtx, CliOptions> {
        self.grateos_fork_ctx.as_mut().unwrap()
    }
}

pub struct DylinkMetadata {
    pub dylink_enabled: bool,
    pub got: Option<Arc<Mutex<GrateOSGOT>>>,
...
/// Global grate-handler registry for grateos-wasm boot/runtime coordination.
///
...
///
/// This logic lives in `grateos-boot`, rather than in `grateos-3i` or Wasmtime,
...
/// layer such as `grateos-3i`, that layer would only see the generic `T` carried
...
///
/// This function must be called exactly once during grateos-wasm startup before
...
pub fn init_grate_pool() {
    GRATE_POOL.get_or_init(|| {
        (0..grateos_platform_const::MAX_CAGEID)
...
fn get_grate_handler(grate_id: u64) -> anyhow::Result<Arc<GrateHandler<HostCtx>>> {
    ...
    println!(
        "[grateos-boot] Retrieving grate handler for grate_id {}",
        grate_id
    );
    let pool = GRATE_POOL
        .get()
    ...
    #[cfg(feature = "debug-grate-calls")]
    println!(
        "[grateos-boot] Grate handler for grate_id {} is {}",
        grate_id,
...
pub fn submit_grate_request(grate_id: u64, req: GrateRequest) -> anyhow::Result<i32> {
    #[cfg(feature = "debug-grate-calls")]
    println!(
        "[grateos-boot] Submitting grate request to cage {}, handler_addr: {:#x}",
    ...
    let handler = match get_grate_handler(grate_id) {
        Ok(handler) => {
            #[cfg(feature = "debug-grate-calls")]
            println!("[grateos-boot] got handler");
            handler
        }
        Err(e) => {
            panic!("[grateos-boot] get_grate_handler failed: {:?}", e);
        }
    };
