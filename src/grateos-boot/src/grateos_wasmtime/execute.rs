use crate::grateos_wasmtime::host::DylinkMetadata;
use crate::grateos_wasmtime::host::{
    cleanup_grate_handler, init_grate_pool, register_grate_handler_for_cage,
    unregister_grate_handler,
};
use crate::{cli::CliOptions, grateos_wasmtime::host::HostCtx, grateos_wasmtime::trampoline::*};
use anyhow::{Context, Result, anyhow, bail};
use cage::signal::{grateos_signal_init, signal_may_trigger};
use cfg_if::cfg_if;
...
use wasmtime_grateos_3i::*;
use wasmtime_grateos_common::GrateOSEnviron;
use wasmtime_grateos_dylink::DynamicLoader;
use wasmtime_grateos_multi_process::{
    CAGE_START_ID, GrateOSCtx, THREAD_START_ID, attach_shared_memory, early_init_stack,
};
use wasmtime_grateos_utils::symbol_table::SymbolMap;
use wasmtime_grateos_utils::{GrateOSCageManager, GrateOSGOT};

/// Boots the GrateOS + RawPOSIX + 3i runtime and executes the initial Wasm program
/// in the first cage.
///
/// This function is the *only* entry point for the initial launch of `grateos-boot`.
...
///
/// Second, it brings up the GrateOS runtime by starting RawPOSIX, creating the first
...
///
/// After initialization, the function attaches all host-side APIs (GrateOS common,
/// WASI threads, and GrateOS multi-process contexts) to the wasmtime linker,
...
pub fn execute_wasmtime(grateosboot_cli: CliOptions) -> anyhow::Result<i32> {
    // -- Initialize GrateOS + RawPOSIX + 3i runtime --
    // Initialize the GrateOS cage counter
    ...
    if !register_wasmtime_syscall_entry() {
        panic!("[grateos-boot] register syscall handlers (clone/exec/exit) with 3i failed");
    }
    ...
    let wt_config = make_wasmtime_config(
        grateosboot_cli.wasmtime_backtrace,
        grateosboot_cli.enable_fpcast,
        grateosboot_cli.grateos_internal_memory_reservation,
    );
    let engine = Engine::new(&wt_config)
        .map_err(anyhow::Error::from)
        .context("failed to create execution engine")?;
    let module = read_main_wasm_or_cwasm(&engine, &grateosboot_cli)?;

    // -- Run the first module in the first cage --
    let result = execute_with_grateos(
        grateosboot_cli,
    ...
    match result {
        ...
        Err(e) => {
            ...
            cage::signal::grateos_thread_exit(cageid, THREAD_START_ID as u64);
...
/// Executes a Wasm program *within an existing GrateOS runtime* as part of an `exec()` path.
///
...
/// global GrateOS/RawPOSIX runtime alive.
///
...
pub fn execute_with_grateos(
    grateos_boot: CliOptions,
    ...
    // -- Initialize the Wasmtime execution environment --
    let args = grateos_boot.args.clone();
    ...
    if dylink_metadata.dylink_enabled {
        ...
        dylink_metadata.got = Some(Arc::new(Mutex::new(GrateOSGOT::new())));
    ...
    for (name, path) in grateos_boot.preloads.iter() {
    ...
    if dylink_metadata.dylink_enabled {
        let grateos_got = dylink_metadata.got.as_ref().unwrap();
        ...
        // register its GOT imports with the shared GrateOSGOT instance.
        //
        ...
        for (name, path, module) in modules.iter().skip(1) {
            ...
            // shared table. GOT entries are patched through the shared GrateOSGOT.
            grateos_log!(DYLINK, "library {} instantiate", name);
            let mut got_guard = grateos_got.lock().unwrap();
            lib_linker
                .module_with_preload(
                    &mut wstore,
...
///
/// During `grateos-boot` initialization, we extract raw function pointers for a
...
/// ```
///   Wasm
///     -> Wasmtime grateos-common trampoline
///     -> 3i dispatch
///     -> RawPOSIX handling
///     -> 3i dispatch
...
/// Attaches all host-side APIs and GrateOS runtime contexts to the linker and store.
///
...
///
/// 1. **GrateOS common** — syscall dispatch, debug helpers, signal hooks, and the
///    4 argv/environ functions that glibc's `_start()` needs.
/// 2. **WASI threads** — pthread-like thread creation within the guest.
/// 3. **GrateOS multi-process** (`GrateOSCtx`) — fork/exec semantics.
///
...
fn attach_api(
    ...
    got_table: Option<Arc<Mutex<GrateOSGOT>>>,
    modules: &Vec<(String, String, Module)>,
    grateos_manager: Arc<GrateOSCageManager>,
    grateosboot_cli: CliOptions,
    cageid: i32,
    dylink_metadata: &DylinkMetadata,
) -> Result<()> {
    // Initialize argv/environ data and attach all GrateOS host functions
    // (syscall dispatch, debug, signals, and argv/environ) to the linker.
    wstore.data_mut().grateos_environ = Some(Arc::new(GrateOSEnviron::new(
        &grateosboot_cli.args,
        &grateosboot_cli.vars,
    )));
        ...
        if dylink_metadata.dylink_enabled {
            let dynamic_loader: DynamicLoader<HostCtx> =
                Arc::new(move |caller, cageid, library_name, mode| {
                    let grateos_ctx = caller.data().grateos_fork_ctx.as_ref().unwrap();
                    let linker = grateos_ctx.linker.clone().unwrap();
                    let got_table = grateos_ctx.got_table.clone().unwrap();

                    if grateos_ctx.had_threads() {
                        grateos_debug_panic!("dlopen within threads is currently not supported!");
                    }
    ...
    let mut linker_guard = linker.lock().unwrap();
    let _ = wasmtime_grateos_common::add_to_linker::<HostCtx, _>(
        &mut linker_guard,
        |s: &HostCtx| {
            s.grateos_environ
                .as_deref()
                .expect("grateos_environ must be initialized")
        },
        dynamic_loader,
    )?;
    ...
    let _ = wstore.data_mut().grateos_fork_ctx = Some(GrateOSCtx::new(
        modules.clone(),
        linker_guard.clone(),
        got_table,
        grateos_manager.clone(),
        grateosboot_cli.clone(),
        cageid,
        grateosboot_cli.thread_stack_size,
        ...
        |grateosboot_cli, path, args, engine, module, cageid, grateos_manager, envs| {
            let mut new_grateosboot_cli = grateosboot_cli.clone();
            new_grateosboot_cli.args = vec![String::from(path)];
            // new_grateosboot_cli.wasm_file = path.to_string();
            if let Some(envs) = envs {
                new_grateosboot_cli.vars = envs.clone();
            }
            for arg in args.iter().skip(1) {
                new_grateosboot_cli.args.push(String::from(arg));
            }

            execute_with_grateos(
                new_grateosboot_cli,
                grateos_manager.clone(),
                engine,
                module,
...
fn load_main_module(
    ...
    if dylink_metadata.dylink_enabled {
        ...
        // after all preloaded library are attached to the linker, update the linker in GrateOSCtx
        // so that newly forked cage could use the Linker with necessary library loaded
        let mut ctx = store.data_mut().grateos_fork_ctx.as_mut().unwrap();
    ...
    cfg_if! {
        ...
        if #[cfg(feature = "disable_signals")] {
            ...
            let epoch_handler = if dylink_metadata.dylink_enabled {
                ...
                grateos_epoch.get_handler_as_u64(&mut *store) as u64
            };
        }
    }
    ...
    // See more details in [grateos-3i/src/lib.rs]
...
///
/// This function implements GrateOS's runtime dynamic loading path (subroutine of `dlopen`).
...
fn load_library_module(
    ...
    mut grateos_got: Arc<Mutex<GrateOSGOT>>,
    cageid: i32,
    library_name: &str,
    dlopen_mode: i32,
) -> i32 {
    ...
    let grateos_ctx = main_module.data_mut().grateos_fork_ctx.as_mut().unwrap();
    grateos_ctx.attach_linker(linker);
    grateos_ctx.append_module(library_name.to_string(), lib_module);
...
pub fn precompile_module(cli: &CliOptions) -> Result<()> {
    ...
    let wt_config = make_wasmtime_config(
        ...
        false,
        cli.grateos_internal_memory_reservation,
    );
    let engine = Engine::new(&wt_config)
        .map_err(anyhow::Error::from)
...
pub(crate) fn read_wasm_or_cwasm_bytes(
    ...
    match Engine::detect_precompiled(bytes) {
        ...
        Some(Precompiled::Component) => bail!(
            "failed to load {source_name}: precompiled components are not supported as GrateOS modules"
        ),
...
/// provided to grateos-boot.
fn make_wasmtime_config(
    backtrace: bool,
    enable_fpcast: bool,
    grateos_internal_memory_reservation: u64,
) -> wasmtime::Config {
    ...
    wt_config.grateos_internal_memory_reservation(grateos_internal_memory_reservation);
    ...
    match Cache::from_file(None) {
        ...
        Err(e) => {
            eprintln!("[grateos-boot] warning: failed to enable wasm cache: {e}");
        }
    }
