...
libc_hidden_def (__register_atfork)
    ...
    static struct fork_handler *fork_handler_list_find (
	struct fork_handler_list *fork_handlers, void *dso_handle)
{
...
__unregister_atfork (void *dso_handle)
{
  ...
  struct fork_handler *first
      = fork_handler_list_find (&fork_handlers, dso_handle);
...
__run_prefork_handlers (_Bool do_locking)
{
  ...
  for (size_t i = sl; i > 0;)
    {
      struct fork_handler *runp = fork_handler_list_at (&fork_handlers, i - 1);
      ...
      if (runp->prepare_handler != NULL)
	{
	 ...
	 any newly added handlers with higher IDs.  */
      while (i > 0 && fork_handler_list_at (&fork_handlers, i - 1)->id >= id)
	i--;
    }
