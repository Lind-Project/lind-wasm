...
read (int __fd, __fortify_clang_overload_arg0 (void *, , __buf),
      size_t __nbytes)
...

{
  return __glibc_fortify (read, __nbytes, sizeof (char),
			  __glibc_objsize0 (__buf), __fd, __buf, __nbytes);
}
...

{
  return __glibc_fortify (readlink, __len, sizeof (char),
			  __glibc_objsize (__buf), __path, __buf, __len);
}
#endif
...
__fortify_function __attribute_overloadable__ __nonnull ((2, 3)) __wur ssize_t
    __NTH (readlinkat (int __fd, const char *__restrict __path,
		       __fortify_clang_overload_arg0 (char *, __restrict,
						      __buf),
		       size_t __len))
	__fortify_clang_warning_only_if_bos_lt (
	    __len, __buf,
	    ...
	    "than size of destination buffer")
{
  return __glibc_fortify (readlinkat, __len, sizeof (char),
			  __glibc_objsize (__buf), __fd, __path, __buf, __len);
}
#endif
					    ...
					    "than size of destination buffer")
{
  return __glibc_fortify (getcwd, __size, sizeof (char),
			  __glibc_objsize (__buf), __buf, __size);
}

#if defined __USE_MISC || defined __USE_XOPEN_EXTENDED
__fortify_function
    __attribute_overloadable__ __nonnull ((1)) __attribute_deprecated__ __wur
    char *__NTH (getwd (__fortify_clang_overload_arg (char *, , __buf)))
{
	...
	"size of destination buffer")
{
  return __glibc_fortify (confstr, __len, sizeof (char),
			  __glibc_objsize (__buf), __name, __buf, __len);
}
	...
	"destination buffer")
{
  return __glibc_fortify (getgroups, __size, sizeof (__gid_t),
			  __glibc_objsize (__list), __size, __list);
}

__fortify_function __attribute_overloadable__ int
__NTH (ttyname_r (int __fd, __fortify_clang_overload_arg (char *, , __buf),
		  size_t __buflen))
    __fortify_clang_warning_only_if_bos_lt (
	__buflen, __buf,
	...
	"than size of destination buffer")
{
  return __glibc_fortify (ttyname_r, __buflen, sizeof (char),
			  __glibc_objsize (__buf), __fd, __buf, __buflen);
}
	...
	"than size of destination buffer")
{
  return __glibc_fortify (getlogin_r, __buflen, sizeof (char),
			  __glibc_objsize (__buf), __buf, __buflen);
}
#endif
