...
#  if WIDE_CHAR_VERSION
  const char *collseq
      = (const char *) _NL_CURRENT (LC_COLLATE, _NL_COLLATE_COLLSEQWC);
#  else
  const UCHAR *collseq
      = (const UCHAR *) _NL_CURRENT (LC_COLLATE, _NL_COLLATE_COLLSEQMB);
#  endif
#endif
	  ...
	  if (c == L_ ('\0'))
		      ...
		      && *p == L_ ('(')))
		{
		  ...
		  for (--p; n < endp; ++n, no_leading_period = false)
		    if (FCT (p, n, string_end, no_leading_period, flags2, &end)
			== 0)
		      goto found;
		}
...
#  if WIDE_CHAR_VERSION
			table = (const int32_t *) _NL_CURRENT (
			    LC_COLLATE, _NL_COLLATE_TABLEWC);
			weights = (const int32_t *) _NL_CURRENT (
			    LC_COLLATE, _NL_COLLATE_WEIGHTWC);
			extra = (const wint_t *) _NL_CURRENT (
			    LC_COLLATE, _NL_COLLATE_EXTRAWC);
			indirect = (const int32_t *) _NL_CURRENT (
			    LC_COLLATE, _NL_COLLATE_INDIRECTWC);
#  else
			table = (const int32_t *) _NL_CURRENT (
			...
			if (idx != 0)
			  {
			    ...
			    idx2 = FINDIDX (table, indirect, extra, &np,
					    string_end - n);
			    if (idx2 != 0 && (idx >> 24) == (idx2 >> 24)
...
#  endif
			    ...
			    table_size = _NL_CURRENT_WORD (
				LC_COLLATE, _NL_COLLATE_SYMB_HASH_SIZEMB);
			    symb_table = (const int32_t *) _NL_CURRENT (
				LC_COLLATE, _NL_COLLATE_SYMB_TABLEMB);
			    extra = (const unsigned char *) _NL_CURRENT (
...
#  if WIDE_CHAR_VERSION
				/* The compile might warn that wextra may be
				   ...
				   */
				DIAG_PUSH_NEEDS_COMMENT;
				DIAG_IGNORE_Os_NEEDS_COMMENT_GCC (
				    8, "-Wmaybe-uninitialized");
     ...
     bool no_leading_period, int flags)
{
  ...
  for (startp = p = pattern + 1; level >= 0; ++p)
	...
	if (level == 0)
	  {
	    size_t slen = opt == L_ ('?') || opt == L_ ('@') ? pattern_len
							     : p - startp + 1;
  ...
  switch (opt)
      ...
      for (; pattern_i < PASTE (PATTERN_PREFIX, _size) (&list); pattern_i++)
	{
	  for (rs = string; rs <= string_end; ++rs)
	    ...
	    if (FCT (*PASTE (PATTERN_PREFIX, _at) (&list, pattern_i), string,
		     ...
		     flags & FNM_FILE_NAME ? flags : flags & ~FNM_PERIOD, NULL)
		    == 0
      ...
      for (rs = string; rs <= string_end; ++rs)
	{
	  ...
	  for (runp_i = pattern_i;
	       runp_i != PASTE (PATTERN_PREFIX, _size) (&list); runp_i++)
	    {
	      if (FCT (*PASTE (PATTERN_PREFIX, _at) (&list, runp_i), string,
		       rs, no_leading_period,
		       flags & FNM_FILE_NAME ? flags : flags & ~FNM_PERIOD,
		       NULL)
		  == 0)
