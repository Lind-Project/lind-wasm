...
static const char *const aio_syms[]
    = { "AIO_ALLDONE", "AIO_CANCELED", "AIO_NOTCANCELED", "LIO_NOP",
	"LIO_NOWAIT", "LIO_READ", "LIO_WAIT", "LIO_WRITE",
	/* From <fcntl.h>.  */
	...
	"O_WRONLY",
	/* From <signal.h>.  */
	"SA_NOCLDSTOP", "SA_SIGINFO", "SIGABRT", "SIGALRM", "SIGBUS",
	"SIGCHLD", "SIGCONT", "SIGEV_NONE", "SIGEV_SIGNAL", "SIGEV_SIGNAL",
	"SIGEV_THREAD", "SIGFPE", "SIGHUP", "SIGILL", "SIGINT", "SIGKILL",
	"SIGPIPE", "SIGQUIT", "SIGRTMAX", "SIGRTMIN", "SIGSEGV", "SIGSTOP",
	"SIGTERM", "SIGTSTP", "SIGTTIN", "SIGTTOU", "SIGUSR1", "SIGUSR2",
	"SIG_BLOCK", "SIG_DFL", "SIG_ERR", "SIG_IGN", "SIG_SETMASK",
	"SIG_UNBLOCK", "SI_ASYNCIO", "SI_MESGQ", "SI_QUEUE", "SI_TIMER",
	"SI_USER" };
static const char *const aio_maybe[]
    = { "aio_cancel", "aio_error", "aio_fsync", "aio_read", "aio_return",
	"aio_suspend", "aio_write", "lio_listio",
	/* From <fcntl.h>.  */
	"creat", "fcntl", "open", "SEEK_CUR", "SEEK_END", "SEEK_SET",
...
/* <assert.h>.  */
static const char *const assert_syms[] = { "assert" };
...
static const char *const errno_syms[]
    = { "E2BIG",    "EACCES",	    "EAGAIN",  "EBADF",	      "EBADMSG",
	"EBUSY",    "ECANCELED",    "ECHILD",  "EDEADLK",     "EDOM",
	"EEXIST",   "EFAULT",	    "EFBIG",   "EINPROGRESS", "EINTR",
	"EINVAL",   "EIO",	    "EISDIR",  "EMFILE",      "EMLINK",
	"EMSGSIZE", "ENAMETOOLONG", "ENFILE",  "ENODEV",      "ENOENT",
	"ENOEXEC",  "ENOLCK",	    "ENOMEM",  "ENOSPC",      "ENOSYS",
	"ENOTDIR",  "ENOTEMPTY",    "ENOTSUP", "ENOTTY",      "ENXIO",
	"EPERM",    "EPIPE",	    "ERANGE",  "EROFS",	      "ESPIPE",
	"ESRCH",    "ETIMEDOUT",    "EXDEV" };
static const char *const errno_maybe[] = { "errno", "E*" };

/* <fcntl.h>.  */
static const char *const fcntl_syms[]
    = { "FD_CLOEXEC", "F_DUPFD", "F_GETFD",   "F_GETFL",    "F_GETLK",
	"F_RDLCK",    "F_SETFD", "F_SETFL",   "F_SETLK",    "F_SETLKW",
	"F_UNLCK",    "F_WRLCK", "O_ACCMODE", "O_APPEND",   "O_CREAT",
	"O_DSYNC",    "O_EXCL",	 "O_NOCTTY",  "O_NONBLOCK", "O_RDONLY",
	"O_RDWR",     "O_RSYNC", "O_SYNC",    "O_TRUNC",    "O_WRONLY" };
static const char *const fcntl_maybe[]
    = { "creat",    "fcntl",   "open",	  "SEEK_CUR", "SEEK_END",
	"SEEK_SET", "S_IRGRP", "S_IROTH", "S_IRUSR",  "S_IRWXG",
	"S_IRWXO",  "S_IRWXU", "S_ISBLK", "S_ISCHR",  "S_ISDIR",
	"S_ISFIFO", "S_ISGID", "S_ISREG", "S_ISUID",  "S_IWGRP",
...
static const char *const limits_syms[]
	...
	"ULONG_MAX",
	"USHRT_MAX" };
static const char *const limits_maybe[] = { "AIO_LISTIO_MAX",
					    "AIO_MAX",
					    "ARG_MAX",
					    "CHILD_MAX",
					    "DELAYTIMER_MAX",
...
/* <math.h>.  */
static const char *const math_syms[] = { "HUGE_VAL" };
static const char *const math_maybe[]
    = { "acos",	  "asin",   "atan2", "atan",   "ceil",	 "cos",	   "cosh",
	"exp",	  "fabs",   "floor", "fmod",   "frexp",	 "ldexp",  "log10",
	"log",	  "modf",   "pow",   "sin",    "sinh",	 "sqrt",   "tan",
	"tanh",	  "acosf",  "asinf", "atan2f", "atanf",	 "ceilf",  "cosf",
	"coshf",  "expf",   "fabsf", "floorf", "fmodf",	 "frexpf", "ldexpf",
	"log10f", "logf",   "modff", "powf",   "sinf",	 "sinhf",  "sqrtf",
	"tanf",	  "tanhf",  "acosl", "asinl",  "atan2l", "atanl",  "ceill",
	"cosl",	  "coshl",  "expl",  "fabsl",  "floorl", "fmodl",  "frexpl",
	"ldexpl", "log10l", "logl",  "modfl",  "powl",	 "sinl",   "sinhl",
	"sqrtl",  "tanl",   "tanhl" };
...
static const char *const pthread_syms[] = {
  ...
  "PTHREAD_CREATE_JOINABLE", "PTHREAD_EXPLICIT_SCHED", "PTHREAD_INHERIT_SCHED",
  "PTHREAD_MUTEX_INITIALIZER", "PTHREAD_ONCE_INIT", "PTHREAD_PRIO_INHERIT",
  "PTHREAD_PRIO_NONE", "PTHREAD_PRIO_PROTECT", "PTHREAD_PROCESS_PRIVATE",
...
static const char *const pthread_maybe[] = {
  ...
  "sched_get_priority_min", "sched_get_rr_interval", "sched_getparam",
...
static const char *const sched_maybe[]
	...
	/* These come from <time.h>.  */
	"CLK_TCK", "CLOCKS_PER_SEC", "CLOCK_REALTIME", "NULL",
	"TIMER_ABSTIME"
	"asctime",
	"asctime_r", "clock", "clock_getres", "clock_gettime", "clock_settime",
	"ctime", "ctime_r", "difftime", "gmtime", "gmtime_r", "localtime",
	"localtime_r", "mktime", "nanosleep", "strftime", "time",
	"timer_create", "timer_delete", "timer_getoverrun", "timer_gettime",
	"timer_settime", "tzset" };
...
static const char *const signal_syms[] = {
  "SA_NOCLDSTOP", "SA_SIGINFO",	 "SIGABRT",	"SIGALRM",	"SIGBUS",
  "SIGCHLD",	  "SIGCONT",	 "SIGEV_NONE",	"SIGEV_SIGNAL", "SIGEV_THREAD",
  "SIGFPE",	  "SIGHUP",	 "SIGILL",	"SIGINT",	"SIGKILL",
  "SIGPIPE",	  "SIGQUIT",	 "SIGRTMAX",	"SIGRTMIN",	"SIGSEGV",
  "SIGSTOP",	  "SIGTERM",	 "SIGTSTP",	"SIGTTIN",	"SIGTTOU",
  "SIGUSR1",	  "SIGUSR2",	 "SIG_BLOCK",	"SIG_DFL",	"SIG_ERR",
  "SIG_IGN",	  "SIG_SETMASK", "SIG_UNBLOCK", "SI_ASYNCIO",	"SI_MESGQ",
  "SI_QUEUE",	  "SI_TIMER",	 "SI_USER"
};
...
/* <stdarg.h>.  */
static const char *const stdarg_syms[] = { "va_arg", "va_end", "va_start" };
static const char *const stdarg_maybe[] = { "va_list" };

/* <stddef.h>.  */
static const char *const stddef_syms[] = { "NULL", "offsetof" };
static const char *const stddef_maybe[] = {};

/* <stdio.h>.  */
static const char *const stdio_syms[]
    = { "BUFSIZ",    "EOF",	   "FILENAME_MAX", "FOPEN_MAX", "L_ctermid",
	"L_cuserid", "L_tmpnam",   "NULL",	   "SEEK_CUR",	"SEEK_END",
	"SEEK_SET",  "STREAM_MAX", "TMP_MAX",	   "stderr",	"stdin",
	"stdout",    "_IOFBF",	   "_IOLBF",	   "_IONBF" };
static const char *const stdio_maybe[] = { "clearerr",
					   "fclose",
					   "fdopen",
					   "feof",
					   "ferror",
...
static const char *const stdlib_maybe[]
	...
	"qsort",   "rand",    "rand_r", "realloc",  "srand",	"strtod",
	"strtol",  "strtoul", "system", "wcstombs", "wctomb" };

/* <string.h>.  */
static const char *const string_syms[] = { "NULL" };
...
static const char *const mman_syms[]
    = { "MAP_FAILED",  "MAP_FIXED",  "MAP_PRIVATE", "MAP_SHARED",
	"MCL_CURRENT", "MCL_FUTURE", "MS_ASYNC",    "MS_INVALIDATE",
	"MS_SYNC",     "PROT_EXEC",  "PROT_NONE",   "PROT_READ",
	"PROT_WRITE" };
static const char *const mman_maybe[]
    = { "mlock",   "mlockall",	 "mmap",   "mprotect", "msync",
	"munlock", "munlockall", "munmap", "shm_open", "shm_unlock" };

/* <sys/stat.h>.  */
static const char *const stat_syms[]
    = { "S_IRGRP",     "S_IROTH",    "S_IRUSR", "S_IRWXG", "S_IRWXO",
	"S_IRWXU",     "S_ISBLK",    "S_ISCHR", "S_ISDIR", "S_ISFIFO",
	"S_ISGID",     "S_ISREG",    "S_ISUID", "S_IWGRP", "S_IWOTH",
	"S_IWUSR",     "S_IXGRP",    "S_IXOTH", "S_IXUSR", "S_TYPEISMQ",
	"S_TYPEISSEM", "S_TYPEISSHM" };
static const char *const stat_maybe[]
    = { "chmod", "fchmod", "fstat", "mkdir", "mkfifo", "stat", "umask" };

/* <sys/times.h>.  */
static const char *const times_syms[] = {};
static const char *const times_maybe[] = { "times" };
...
static const char *const utsname_maybe[] = { "uname" };

/* <sys/wait.h>.  */
static const char *const wait_syms[]
    = { "WEXITSTATUS", "WIFEXITED", "WIFSIGNALED", "WIFSTOPPED",
	"WNOHANG",     "WSTOPSIG",  "WTERMSIG",	   "WUNTRACED" };
static const char *const wait_maybe[] = { "wait", "waitpid" };

/* <termios.h>.  */
static const char *const termios_syms[]
    = { "B0",	     "B110",	  "B1200",     "B134",	   "B150",   "B1800",
	"B19200",    "B200",	  "B2400",     "B300",	   "B38400", "B4800",
	"B50",	     "B600",	  "B75",       "B9600",	   "BRKINT", "CLOCAL",
	"CREAD",     "CS5",	  "CS6",       "CS7",	   "CS8",    "CSIZE",
	"CSTOPN",    "ECHO",	  "ECHOE",     "ECHOK",	   "ECHONL", "HUPCL",
	"ICANON",    "ICRNL",	  "IEXTEN",    "IGNBRK",   "IGNCR",  "IGNPAR",
	"INCLR",     "INPCK",	  "ISIG",      "ISTRIP",   "IXOFF",  "IXON",
	"NCCS",	     "NOFLSH",	  "OPOST",     "PARENB",   "PARMRK", "PARODD",
	"TCIFLUSH",  "TCIOFF",	  "TCIOFLUSH", "TCOFLUSH", "TCOOFF", "TCOON",
	"TCSADRAIN", "TCSAFLUSH", "TCSANOW",   "TOSTOP",   "VEOF",   "VEOL",
	"VERASE",    "VINTR",	  "VKILL",     "VMIN",	   "VQUIT",  "VSTART",
	"VSTOP",     "VSUSP",	  "VTIME" };
...
static const char *const time_maybe[]
    = { "asctime",	 "asctime_r",	  "clock",	  "clock_getres",
	"clock_gettime", "clock_settime", "ctime",	  "ctime_r",
	"difftime",	 "gmtime",	  "gmtime_r",	  "localtime",
	"localtime_r",	 "mktime",	  "nanosleep",	  "strftime",
	"time",		 "timer_create",  "timer_delete", "timer_getoverrun",
	"timer_gettime", "timer_settime", "tzset" };

/* <unistd.h>.  */
static const char *const unistd_syms[] = { "F_OK",
					   "NULL",
					   "R_OK",
					   "SEEK_CUR",
					   "SEEK_END",
...
static const char *const utime_maybe[] = { "utime" };
...
#define H(n)                                                                  \
  { #n ".h",                                                                  \
    n##_syms,                                                                 \
    sizeof (n##_syms) / sizeof (n##_syms[0]),                                 \
    n##_maybe,                                                                \
    sizeof (n##_maybe) / sizeof (n##_maybe[0]),                               \
    NULL }
#define Hc(n, s)                                                              \
  { #n ".h",                                                                  \
    n##_syms,                                                                 \
...
#define Hs(n)                                                                 \
  { "sys/" #n ".h",                                                           \
    ...
    sizeof (n##_maybe) / sizeof (n##_maybe[0]),                               \
    NULL }
  H (aio),     H (assert),  H (ctype),	H (dirent),
  H (errno),   H (fcntl),   H (float),	H (grp),
  H (limits),  H (locale),  H (math),	Hc (mqueue, "_POSIX_MESSAGE_PASSING"),
  H (pthread), H (pwd),	    H (sched),	H (semaphore),
  H (setjmp),  H (signal),  H (stdarg), H (stddef),
  H (stdio),   H (stdlib),  H (string), Hs (mman),
  Hs (stat),   Hs (times),  Hs (types), Hs (utsname),
  Hs (wait),   H (termios), H (time),	H (unistd),
  H (utime)
};
...
get_null_defines (void)
{
  ...
  while (fgets (line, sizeof line, input) != NULL)
      ...
      if (result_len == result_max)
	{
      ...
      if (first)
	{
	  fputs ("The following identifiers will be ignored since the "
...
{
  char line[BUFSIZ],
      command[sizeof fmt + strlen (header->name) + 2 * strlen (CC)
	      + strlen (INC) + strlen (macrofile)];
  FILE *input;
  int result = 0;
