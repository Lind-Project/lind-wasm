...
do_one_test (const struct test_s *test, const char *fail)
{
  ...
  err = re_compile_pattern (test->pattern, strlen (test->pattern), &regbuf);
  if (err != NULL)
    {
...
#endif
  res = re_search (&regbuf, test->string, strlen (test->string), test->start,
		   strlen (test->string) - test->start, NULL);
#if __GNUC_PREREQ(10, 0) && !__GNUC_PREREQ(11, 0)
  ...
  if (res != test->res)
    {
      printf ("%sre_search \"%s\" \"%s\" failed: %d (expected %d)\n", fail,
	      test->pattern, test->string, res, test->res);
      regfree (&regbuf);
      return 1;
    }
  ...
  if (test->res > 0 && test->start == 0)
    {
	 ...
	 object size.  */
      DIAG_IGNORE_NEEDS_COMMENT (10, "-Wstringop-overflow");
#endif
      res = re_search (&regbuf, test->string, strlen (test->string), test->res,
		       strlen (test->string) - test->res, NULL);
#if __GNUC_PREREQ(10, 0) && !__GNUC_PREREQ(11, 0)
      DIAG_POP_NEEDS_COMMENT;
#endif
      if (res != test->res)
	{
	  printf ("%sre_search from expected \"%s\" \"%s\" failed: %d "
		  "(expected %d)\n",
