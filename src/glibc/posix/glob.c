...
#define READDIR_RESULT_INITIALIZER(source)                                    \
  { source->d_name, D_TYPE_TO_RESULT (source) }
	...
	glob_t *pglob)
{
  ...
  if (!(flags & GLOB_APPEND))
      ...
      else
	{
	  ...
	  pglob->gl_pathv
	      = (char **) malloc ((pglob->gl_offs + 1) * sizeof (char *));
	  ...
	  while (1)
	    {
	      ...
	      result = __glob (
		  onealt,
		  ((flags & ~(GLOB_NOCHECK | GLOB_NOMAGIC)) | GLOB_APPEND),
		  errfunc, pglob);
...
#  endif
	  ...
	  if (dirname[1] == '\0')
	      ...
	      else
		{
	      ...
	      mempcpy (mempcpy (newp, home_dir, home_len), &dirname[1],
		       dirlen);
      ...
      else
	{
	  ...
	  if (end_name == NULL)
		...
		{
		  char *p
		      = mempcpy (newp, dirname + 1, unescape - dirname - 1);
		  char *q = unescape;
		  while (q != end_name)
		    {
		      if (*q == '\\')
...
#  if defined HAVE_GETPWNAM_R || defined _LIBC
	    ...
	    while (getpwnam_r (user_name, &pwbuf, pwtmpbuf.data,
			       pwtmpbuf.length, &p)
		   == ERANGE)
	      {
...
#  endif
		...
		if (glob_use_alloca (alloca_used, home_len + rest_len + 1))
		  dirname
		      = alloca_account (home_len + rest_len + 1, alloca_used);
		else
		  {
  ...
  if (filename == NULL)
	...
	nospace:
      ...
      new_gl_pathv
	  = realloc (pglob->gl_pathv, (newcount + 2) * sizeof (char *));
  ...
  if (meta & (GLOBPAT_SPECIAL | GLOBPAT_BRACKET))
      ...
      for (i = 0; i < dirs.gl_pathc; ++i)
	{
	  ...
	  status = glob_in_dir (
	      filename, dirs.gl_pathv[i],
	      ((flags | GLOB_APPEND) & ~(GLOB_NOCHECK | GLOB_NOMAGIC)),
  ...
  if (!(flags & GLOB_NOSORT))
    {
      ...
      qsort (&pglob->gl_pathv[oldcount],
	     pglob->gl_pathc + pglob->gl_offs - oldcount, sizeof (char *),
	     collated_compare);
    }
...
#define GLOBNAMES_MEMBERS(nnames)                                             \
  ...
  init_names->count
      = ((sizeof init_names_buf - offsetof (struct globnames, name))
  ...
  if (meta == GLOBPAT_NONE && (flags & (GLOB_NOCHECK | GLOB_NOMAGIC)))
      ...
      else
	{
      ...
      mempcpy (mempcpy (mempcpy (fullname, directory, dirlen), "/", 1),
	       pattern, patlen + 1);
      if (glob_lstat (pglob, flags, fullname) == 0 || errno == EOVERFLOW)
