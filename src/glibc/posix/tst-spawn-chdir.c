  ...
  if (do_fchdir)
    {
      TEST_COMPARE (posix_spawn_file_actions_addopen (
			actions, tmpfd, path, O_DIRECTORY | O_RDONLY, 0),
		    0);
      TEST_COMPARE (posix_spawn_file_actions_addfchdir_np (actions, tmpfd), 0);
...
do_test (void)
{
  ...
  for (int do_spawnp = 0; do_spawnp < 2; ++do_spawnp)
      ...
      for (int do_fchdir = 0; do_fchdir < 2; ++do_fchdir)
	{
	  ...
	  char *iteration_directory
	      = xasprintf ("%s/%d", directory, iteration);
	  ...
	  TEST_COMPARE (posix_spawn_file_actions_addopen (
			    ...
			    "probe-file", O_WRONLY | O_CREAT | O_EXCL, 0777),
			0);
	  ...
	  if (do_spawnp)
	    TEST_COMPARE (
		POSIX_SPAWNP (&pid, "pwd", &actions, NULL, argv, envp), 0);
	  else
	  ...
	  /* Check that the current directory did not change.  */
	  {
	    ...
	    if (cwd == NULL)
	      FAIL_EXIT1 ("get_current_dir_name: %m");
	    TEST_COMPARE_BLOB (original_cwd, strlen (original_cwd), cwd,
			       strlen (cwd));
	    free (cwd);
	  }
	  ...
	  /* Check the output from "pwd".  */
	  {
	    ...
	    TEST_COMPARE_BLOB (iteration_directory,
			       strlen (iteration_directory), pwd,
			       strlen (pwd));
	    free (pwd);
	  }
