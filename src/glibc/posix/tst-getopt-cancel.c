...
#define DEFINE_TEST_DRIVER(test_type, getopt_call)                            \
  ...
  {                                                                           \
    ...
    bool ok;                                                                  \
                                                                              ...
                                                                              \
  static void *test_type##_threadproc (void *data)                            \
  {                                                                           \
    ...
    return 0;                                                                 \
                                                                              ...
                                                                              \
  static bool do_##test_type (const struct test_type *tcase,                  \
			      FILE *stderr_trapped)                           \
  {                                                                           \
    pthread_mutex_t sync;                                                     \
    ...
    tdata.ok = false;                                                         \
                                                                              \
    pthread_t thr                                                             \
	= xpthread_create (0, test_type##_threadproc, (void *) &tdata);       \
    xpthread_cancel (thr);                                                    \
...
DEFINE_TEST_DRIVER (test_long, getopt_long (tc->argc, (char *const *) tc->argv,
...
const struct test_short tests_short[]
    = { { "no errors", "+ab:c", { "program", "-ac", "-b", "x", 0 }, 4, false },
	{ "invalid option", "+ab:c", { "program", "-d", 0 }, 2, true },
	{ "missing argument", "+ab:c", { "program", "-b", 0 }, 2, true },
	{ 0 } };
...
const struct test_long tests_long[]
	...
	{ "unwanted argument",
	  ...
	  { { "alpha", no_argument, 0, 'a' },
	    ...
	    { "charlie", no_argument, 0, 'c' },
	    { 0 } },
	  { "program", "-a", "--charlie=dingo", "--bravo=x", 0 },
	  4,
	  true },
	...
	{ "missing argument",
	  ...
	  { { "alpha", no_argument, 0, 'a' },
	    ...
	    { "charlie", no_argument, 0, 'c' },
	    { 0 } },
	  { "program", "-a", "--charlie", "--bravo", 0 },
	  4,
	  true },

	{ "ambiguous options",
	  "+uvw",
	  { { "veni", no_argument, 0, 'u' },
	...
	{ "no errors (long W)",
	  ...
	  { { "alpha", no_argument, 0, 'a' },
	    ...
	    { "charlie", no_argument, 0, 'c' },
	    { 0 } },
	  { "program", "-a", "-W", "charlie", "-W", "bravo=x", 0 },
	  6,
	  false },
	...
	{ "unwanted argument (W longopt)",
	  ...
	  { { "alpha", no_argument, 0, 'a' },
	    ...
	    { "charlie", no_argument, 0, 'c' },
	    { 0 } },
	  { "program", "-a", "-W", "charlie=dingo", "-W", "bravo=x", 0 },
	  6,
	  true },
