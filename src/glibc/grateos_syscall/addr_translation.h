#ifndef _GRATEOS_ADDR_TRANSLATION_H
#define _GRATEOS_ADDR_TRANSLATION_H

#include <stdint.h>
#include <stddef.h>
#include <errno.h>

// With the new semantics, a cageid with its MSB set indicates that the
// associated argument should be translated. These helpers manage that flag.
//
// Flag indicating the argument requires translation (MSB set).
#define GRATEOS_ARG_TRANSLATE_FLAG (1ULL << 63)

// Mask to extract the actual cageid (clear the translation flag).
#define GRATEOS_ARG_CAGEID_MASK (~GRATEOS_ARG_TRANSLATE_FLAG)

#ifdef __cplusplus
extern "C"
{
#endif

  // Imported host function to get the base address of the current cage's
  // linear memory Module: "grateos", name: "grateos-get-memory-base"
  unsigned long long __imported_grateos_get_memory_base (void)
      __attribute__ ((__import_module__ ("grateos"),
		      __import_name__ ("grateos-get-memory-base")));

  // Imported host function to get the current cage id (pid)
  // Module: "grateos", name: "grateos-get-cage-id"
  unsigned long long __imported_grateos_get_cage_id (void)
      __attribute__ ((__import_module__ ("grateos"),
		      __import_name__ ("grateos-get-cage-id")));

  // Cached base address for this process (cage). Set once per instance.
  extern uint64_t __grateos_base;

  // Cached cage id (pid) for this process (cage). Set once per instance.
  extern uint64_t __grateos_cageid;

  // Initialize address translation (idempotent). Queries base from host once.
  void __grateos_init_addr_translation (void);

  // Translate a guest pointer (offset in wasm32 linear memory) to a host
  // pointer (u64)
  static inline uint64_t
  __grateos_translate_ptr_to_host (const void *p)
  {
    if (p == NULL)
      return 0ULL;
    // Cast pointer value as an offset within the linear memory and add base
    return __grateos_base + (uint64_t) (uintptr_t) p;
  }

  static inline uint64_t
  __grateos_translate_uaddr_to_host (const uint64_t uaddr, const uint64_t cageid)
  {
    // Extract actual cageid (without the translation flag)
    uint64_t __cageid = cageid & GRATEOS_ARG_CAGEID_MASK;

    // Translate only if:
    // 1. The argument originates from the current cage, and
    // 2. MSB of cageid is set.
    if (__cageid == __grateos_cageid && ((cageid & GRATEOS_ARG_TRANSLATE_FLAG) != 0))
      return __grateos_base + uaddr;

    return uaddr;
  }

// Convenience macro for call sites
#define TRANSLATE_GUEST_POINTER_TO_HOST(p)                                    \
  __grateos_translate_ptr_to_host ((const void *) (p))

// Convert a (uaddr, cageid) pair to (host_addr, cageid).
//
// Used by copy_data_between_cages where arguments are already addresses. This macro
// sets the translation flag before invoking the helper.
//
// Input:  (uaddr, cageid)
// Output: (host_addr, cageid)
#define TRANSLATE_UADDR_TO_HOST(uaddr, cageid)                                \
  __grateos_translate_uaddr_to_host ((uaddr), ((cageid) | GRATEOS_ARG_TRANSLATE_FLAG)), (cageid)

// Used by make_threei_call to translate arguments if required.
//
// Translation occurs only when the MSB of cageid is set.
//
// Input:  (uaddr, cageid)
// Output: (host_addr, actual_cageid)
#define TRANSLATE_ARG_TO_HOST(uaddr, cageid)                                  \
  __grateos_translate_uaddr_to_host ((uaddr), (cageid)), ((cageid) & GRATEOS_ARG_CAGEID_MASK)

/* Translate an array of guest iovec structures to host layout.
   Each iov_base is a wasm32 guest pointer; we split the translated
   64-bit host address across iov_base (low32) and __padding1 (high32).
   Requires <sys/uio.h> (or <bits/types/struct_iovec.h>) for struct iovec.  */
#ifdef __iovec_defined
  static inline void
  __grateos_translate_iov (const struct iovec *guest_iov,
                        struct iovec *host_iov, int iovcnt)
  {
    for (int i = 0; i < iovcnt; ++i)
      {
        host_iov[i].iov_len = guest_iov[i].iov_len;
        uint32_t g = (uint32_t)(uintptr_t) guest_iov[i].iov_base;
        uint64_t h = __grateos_translate_ptr_to_host ((const void *)(uintptr_t) g);
        host_iov[i].iov_base   = (void *)(uintptr_t)(uint32_t)(h & 0xFFFFFFFFULL);
        host_iov[i].__padding1 = (int)(uint32_t)(h >> 32);
        host_iov[i].__padding2 = 0;
      }
  }
#endif

#ifdef __cplusplus
}
#endif

#endif // _GRATEOS_ADDR_TRANSLATION_H
