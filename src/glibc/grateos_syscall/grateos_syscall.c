...
// Entry point for wasmtime, grateos_syscall is an imported function from wasmtime
int __grateos_make_syscall_trampoline(unsigned int callnumber, 
    uint64_t callname, 
 ...
 * core entry point for all syscall transitions into the grateos runtime,
 ...
 * grateos_syscall should apply standard POSIX errno translation or return
 ...
 *   (2) the actual Wasmtime entry function (__grateos_make_syscall_trampoline).
 *
...
int make_threei_call (unsigned int callnumber, 
    ...
    int translate_errno)
{
    int ret = __grateos_make_syscall_trampoline(callnumber, 
        callname, 
