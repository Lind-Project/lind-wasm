...
do_test (void)
{
  ...
  {
    int *page
	= xmmap (NULL, pagesize, PROT_NONE, MAP_ANONYMOUS | MAP_PRIVATE, -1);
  ...
  pthread_t delayed_thread = xpthread_create (NULL, &delayed_thread_func,
  ...
  if (keys[0] < 0)
    {
      if (errno == ENOSYS)
	FAIL_UNSUPPORTED ("kernel does not support memory protection keys");
      if (errno == EINVAL)
	FAIL_UNSUPPORTED ("CPU does not support memory protection keys: %m");
      if (errno == ENOSPC)
	FAIL_UNSUPPORTED ("no keys available or kernel does not support memory"
			  " protection keys");
     ...
     keys.  */
  {
    ...
    for (int i = 0; i < key_count; ++i)
      TEST_COMPARE (result->access_rights[i], PKEY_DISABLE_ACCESS);
  ...
  delayed_thread = xpthread_create (NULL, delayed_thread_func,
				    &delayed_thread_check_access);
