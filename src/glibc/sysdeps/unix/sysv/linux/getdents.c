...
{
   return MAKE_SYSCALL(GETDENTS_SYSCALL, "syscall|getdents", (uint64_t) fd, (uint64_t) TRANSLATE_GUEST_POINTER_TO_HOST(buf0), (uint64_t) nbytes, NOTUSED, NOTUSED, NOTUSED);
}
