...
/* WARNING: TREG must be a callee saves register so
   ...
   to another function */
#  define TREG 4
#  define SAVE_PIC(SREG) copy % r19, SREG
#  define LOAD_PIC(LREG) copy LREG, % r19
...
#    undef ENTRY
#    define ENTRY(name)                                                       \
      .text ASM_LINE_SEP.align ALIGNARG (4)                                   \
	  ASM_LINE_SEP.export C_SYMBOL_NAME (name) ASM_LINE_SEP.type          \
	  C_SYMBOL_NAME (name),                                               \
	  @function ASM_LINE_SEP cfi_startproc ASM_LINE_SEP C_LABEL (name)    \
	      ASM_LINE_SEP.PROC ASM_LINE_SEP.CALLINFO FRAME                   \
	  = 64,                                                               \
	  CALLS, SAVE_RP,                                                     \
	  ENTRY_GR                                                            \
	  = 3 ASM_LINE_SEP                                                    \
		.ENTRY ASM_LINE_SEP /* SAVE_RP says we do */ ASM_LINE_SEP stw \
	    % rp,                                                             \
	  -20(% sr0, % sp) ASM_LINE_SEP.cfi_offset 2,                         \
	  -20 ASM_LINE_SEP /*FIXME: Call mcount? (careful with stack!) */
...
#    undef END
#    define END(name)                                                         \
      .EXIT ASM_LINE_SEP.PROCEND ASM_LINE_SEP cfi_endproc ASM_LINE_SEP.size   \
      C_SYMBOL_NAME (name),                                                   \
	  .- C_SYMBOL_NAME (name) ASM_LINE_SEP
...
#    define PSEUDO(name, syscall_name, args)                                  \
      ENTRY (name)                                                            \
      ASM_LINE_SEP /* If necc. load args from stack */ ASM_LINE_SEP           \
...
#    define PSEUDO_NOERRNO(name, syscall_name, args)                          \
      ENTRY_LEAF (name)                                                       \
      ASM_LINE_SEP DOARGS_##args ASM_LINE_SEP DO_CALL_NOERRNO (syscall_name,  \
							       args)          \
      ASM_LINE_SEP                                                            \
      UNDOARGS_##args ASM_LINE_SEP
...
#    define PSEUDO_ERRVAL(name, syscall_name, args)                           \
      ENTRY_LEAF (name)                                                       \
      ASM_LINE_SEP DOARGS_##args ASM_LINE_SEP DO_CALL_ERRVAL (syscall_name,   \
							      args)           \
      ASM_LINE_SEP                                                            \
      UNDOARGS_##args ASM_LINE_SEP
...
#    define DEFAULT_SYSCALL_ERROR_HANDLER                                     \
	  ...
	  code ASM_LINE_SEP /* branch to errno handler */ ASM_LINE_SEP bl     \
	      __errno_location,                                               \
	  % rp ASM_LINE_SEP
...
#    define DO_CALL(syscall_name, args)                                       \
      ...
      stwm TREG,                                                              \
	  64(% sp) ASM_LINE_SEP.cfi_def_cfa_offset                            \
	      - 64 ASM_LINE_SEP.cfi_offset TREG,                              \
	  0 ASM_LINE_SEP stw % sp, -4(% sp) ASM_LINE_SEP stw % r19,           \
	  -32(% sp) ASM_LINE_SEP.cfi_offset 19,                               \
	  32 ASM_LINE_SEP /* Save r19 */ ASM_LINE_SEP SAVE_PIC (TREG)         \
      ASM_LINE_SEP                                                            \
      /* Do syscall, delay loads # */ ASM_LINE_SEP                            \
      ble 0x100(% sr2, % r0) ASM_LINE_SEP ldi SYS_ify (syscall_name),         \
	  % r20 ASM_LINE_SEP ldi NO_ERROR, % r1 ASM_LINE_SEP cmpb, >>=,       \
	  n % r1, % ret0,                                                     \
	  L (pre_end) ASM_LINE_SEP /* Restore r19 from TREG */ ASM_LINE_SEP   \
	      LOAD_PIC (TREG) /* delay */                                     \
      ASM_LINE_SEP                                                            \
      SYSCALL_ERROR_HANDLER ASM_LINE_SEP /* Use TREG for temp storage */      \
	      ASM_LINE_SEP copy                                               \
	  % ret0,                                                             \
	  TREG /* delay */ ASM_LINE_SEP /* OPTIMIZE: Don't reload r19 */      \
		  ASM_LINE_SEP /* do a -1*syscall_ret0 */ ASM_LINE_SEP sub    \
	      % r0,                                                           \
	  TREG,                                                               \
	  TREG ASM_LINE_SEP /* Store into errno location */ ASM_LINE_SEP stw  \
	      TREG,                                                           \
	  0(% sr0, % ret0)                                                    \
		  ASM_LINE_SEP /* return -1 as error */ ASM_LINE_SEP ldo      \
	      - 1(% r0),                                                      \
	  % ret0 ASM_LINE_SEP L (pre_end)                                     \
	  : ASM_LINE_SEP /* Restore our frame, restoring TREG */ ASM_LINE_SEP \
		ldwm                                                          \
	      - 64(% sp),                                                     \
      TREG ASM_LINE_SEP /* Restore return pointer */ ASM_LINE_SEP ldw         \
	  - 20(% sp),                                                         \
      % rp ASM_LINE_SEP
...
#    define DO_CALL_NOERRNO(syscall_name, args)                               \
      ...
      ble 0x100(% sr2, % r0) ASM_LINE_SEP ldi SYS_ify (syscall_name),         \
	  % r20 ASM_LINE_SEP /* Caller will restore r19 */ ASM_LINE_SEP
...
#    define DO_CALL_ERRVAL(syscall_name, args)                                \
      /* No need to store r19 */ ASM_LINE_SEP                                 \
      ble 0x100(% sr2, % r0) ASM_LINE_SEP ldi SYS_ify (syscall_name),         \
	  % r20 ASM_LINE_SEP /* Caller will restore r19 */ ASM_LINE_SEP ldi   \
		  NO_ERROR,                                                   \
	  % r1 ASM_LINE_SEP cmpb, >>=, n % r1, % ret0,                        \
	  0f ASM_LINE_SEP sub % r0, % ret0,                                   \
	  % ret0 ASM_LINE_SEP 0 : ASM_LINE_SEP
...
#    define CALL_CLOB_REGS "%r1", "%r2", CLOB_TREG "%r20", "%r29", "%r31"
...
#    define INTERNAL_SYSCALL(name, nr, args...)                               \
	...
	long __sys_res;                                                       \
	{                                                                     \
	  LOAD_ARGS_##nr (args) register unsigned long __res asm ("r28");     \
	  PIC_REG_DEF                                                         \
	  LOAD_REGS_##nr /* FIXME: HACK save/load r19 around syscall */       \
	      __sys_res                                                       \
	      = (long) __res;                                                 \
	}                                                                     \
	__sys_res;                                                            \
