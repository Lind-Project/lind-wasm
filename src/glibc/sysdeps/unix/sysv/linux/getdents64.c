...
__getdents64 (int fd, void *buf, size_t nbytes)
{
  ...
  // Added MAKE_SYSCALL macro to interface with Lind - Qianxi Chen
	return MAKE_SYSCALL(GETDENTS_SYSCALL, "syscall|getdents", (uint64_t) fd, (uint64_t) TRANSLATE_GUEST_POINTER_TO_HOST(buf), (uint64_t) nbytes, NOTUSED, NOTUSED, NOTUSED);
}
