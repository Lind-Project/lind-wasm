...
{
  return MAKE_SYSCALL(PREAD_SYSCALL, "syscall|pread", (uint64_t) fd, (uint64_t) TRANSLATE_GUEST_POINTER_TO_HOST(buf), (uint64_t) count, (uint64_t) offset, NOTUSED, NOTUSED);
}
