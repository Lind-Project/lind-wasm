...
#    define PSEUDO(name, syscall_name, args)                                  \
      ...
      ASM_LINE_SEP brhi r0, -4096, L (call_syscall_err) ASM_LINE_SEP

#    define ret j_s[blink]

#    undef PSEUDO_END
#    define PSEUDO_END(name) SYSCALL_ERROR_HANDLER ASM_LINE_SEP END (name)
...
#    define ret_NOERRNO j_s[blink] ASM_LINE_SEP
...
#    define ret_ERRVAL rsub r0, r0, 0 ASM_LINE_SEP ret_NOERRNO
...
#    if IS_IN(libc)
#      define CALL_ERRNO_SETTER_C                                             \
	bl PLTJMP (HIDDEN_JUMPTARGET (__syscall_error))
#    else
...
#    define SYSCALL_ERROR_HANDLER                                             \
      L (call_syscall_err)                                                    \
	  : ASM_LINE_SEP push_s blink ASM_LINE_SEP cfi_adjust_cfa_offset (4)  \
		ASM_LINE_SEP                                                  \
		cfi_rel_offset (blink, 0)                                     \
		    ASM_LINE_SEP CALL_ERRNO_SETTER_C ASM_LINE_SEP pop_s blink \
			ASM_LINE_SEP cfi_adjust_cfa_offset (-4)               \
			    ASM_LINE_SEP cfi_restore (blink)                  \
      ASM_LINE_SEP                                                            \
      j_s[blink]

#    define DO_CALL(syscall_name, args)                                       \
      mov r8, __NR_##syscall_name ASM_LINE_SEP ARC_TRAP_INSN ASM_LINE_SEP
...
#    define LOAD_ARGS_0(nm, arg) _sys_num = (long int) (nm);
