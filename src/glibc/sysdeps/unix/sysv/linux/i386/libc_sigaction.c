...
__libc_sigaction (int sig, const struct sigaction *act, struct sigaction *oact)
{
  ...
  int retval = MAKE_LEGACY_SYSCALL(SIGACTION_SYSCALL, "syscall|sigaction", (uint64_t) sig, (uint64_t) TRANSLATE_GUEST_POINTER_TO_HOST(act ? &rawposix_act : NULL), (uint64_t) TRANSLATE_GUEST_POINTER_TO_HOST(oact ? &rawposix_oact : NULL), NOTUSED, NOTUSED, NOTUSED, WRAPPED_SYSCALL);
