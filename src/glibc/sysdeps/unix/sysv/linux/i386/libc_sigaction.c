...
{
  /* manual translation between glibc sigaction struct and rawposix sigaction
   * struct */
  ...
  int retval = MAKE_SYSCALL (
      ...
      (uint64_t) TRANSLATE_GUEST_POINTER_TO_HOST (act ? &rawposix_act : NULL),
      (uint64_t) TRANSLATE_GUEST_POINTER_TO_HOST (oact ? &rawposix_oact
						       : NULL),
