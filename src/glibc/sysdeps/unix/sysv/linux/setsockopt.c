...
return MAKE_LEGACY_SYSCALL(SETSOCKOPT_SYSCALL, "syscall|setsockopt", (uint64_t) fd, (uint64_t) level, (uint64_t) optname, host_optval, len, (uint64_t)0, WRAPPED_SYSCALL);
}
