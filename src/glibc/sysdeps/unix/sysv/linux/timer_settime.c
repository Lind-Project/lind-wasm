...
#    endif
  return INLINE_SYSCALL_CALL (timer_settime64, ktimerid, flags, value, ovalue);
#  else
#    ifdef __NR_timer_settime64
  int ret
      = INLINE_SYSCALL_CALL (timer_settime64, ktimerid, flags, value, ovalue);
...
#    endif
  ...
  int retval = INLINE_SYSCALL_CALL (timer_settime, ktimerid, flags, &its32,
				    ovalue ? &oits32 : NULL);
  if (retval == 0 && ovalue)
    {
...
libc_hidden_proto (__timer_settime_new)
    ...
    int ___timer_settime_new (timer_t timerid, int flags,
...
__timer_settime_old (int timerid, int flags, const struct itimerspec *value,
		     struct itimerspec *ovalue)
{
  return __timer_settime_new (__timer_compat_list[timerid], flags, value,
			      ovalue);
}
