...
#  define LOCK_RW                                                             \
    192 /* ... Which allows concurrent read & write operations.  */
#endif
...
#  define F_SEAL_FUTURE_WRITE                                                 \
    0x0010		     /* Prevent future writes while                   \
				mapped.  */
...
#  define POSIX_FADV_DONTNEED                                                 \
    __POSIX_FADV_DONTNEED /* Don't need these pages.  */
#  define POSIX_FADV_NOREUSE                                                  \
    __POSIX_FADV_NOREUSE /* Data will be accessed once.  */
#endif

#ifdef __USE_GNU
/* Flags for SYNC_FILE_RANGE.  */
#  define SYNC_FILE_RANGE_WAIT_BEFORE                                         \
    1 /* Wait upon writeout of all pages                                      \
	 in the range before performing the                                   \
	 write.  */
#  define SYNC_FILE_RANGE_WRITE                                               \
    2 /* Initiate writeout of all those                                       \
	 dirty pages in the range which are                                   \
	 not presently under writeback.  */
#  define SYNC_FILE_RANGE_WAIT_AFTER                                          \
    4 /* Wait upon writeout of all pages in                                   \
...
#  define SPLICE_F_NONBLOCK                                                   \
    2			  /* Don't block on the pipe splicing                 \
...
#  define AT_HANDLE_FID                                                       \
    AT_REMOVEDIR /* File handle is needed                                     \
		    to compare object                                         \
...
extern __ssize_t readahead (int __fd, __off64_t __offset,
...
#  else
#    ifdef __REDIRECT
extern int __REDIRECT (fallocate,
		       (int __fd, int __mode, __off64_t __offset,
			__off64_t __len),
		       fallocate64);
#    else
