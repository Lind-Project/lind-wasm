...
{
   return MAKE_SYSCALL(UNLINKAT_SYSCALL, "syscall|unlinkat", (uint64_t) dirfd, (uint64_t) TRANSLATE_GUEST_POINTER_TO_HOST(name), (uint64_t) flags, NOTUSED, NOTUSED, NOTUSED);
}
