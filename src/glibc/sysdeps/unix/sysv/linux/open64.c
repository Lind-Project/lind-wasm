...
__libc_open64 (const char *file, int oflag, ...)
{
  ...
  return MAKE_SYSCALL(OPEN_SYSCALL, "syscall|open", (uint64_t) TRANSLATE_GUEST_POINTER_TO_HOST(file), (uint64_t) oflag | O_LARGEFILE, (uint64_t) mode, NOTUSED, NOTUSED, NOTUSED);
}
