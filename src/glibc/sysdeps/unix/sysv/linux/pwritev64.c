...
#else
static ssize_t __atomic_pwritev64_replacement (int, const struct iovec *, int,
					       off64_t);
ssize_t
pwritev64 (int fd, const struct iovec *vector, int count, off64_t offset)
{
  ssize_t result
      = SYSCALL_CANCEL (pwritev, fd, vector, count, LO_HI_LONG (offset));
...
libc_hidden_def (pwritev64)
    ...
    strong_alias (pwritev64, pwritev) libc_hidden_def (pwritev)
#endif
