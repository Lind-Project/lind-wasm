...
#    define atomic_compare_and_exchange_bool_acq(mem, new, old)               \
      __atomic_bool_bysize (__arch_compare_and_exchange_bool, int, mem, new,  \
			    old, __ATOMIC_ACQUIRE)

#    define atomic_compare_and_exchange_val_acq(mem, new, old)                \
      __atomic_val_bysize (__arch_compare_and_exchange_val, int, mem, new,    \
			   old, __ATOMIC_ACQUIRE)
...
#    define asm_amo(which, ordering, mem, value)                              \
	...
	typeof (*mem) __tmp;                                                  \
	if (sizeof (__tmp) == 4)                                              \
	  else if (sizeof (__tmp) == 8) else abort ();                        \
	__tmp;                                                                \
      })
