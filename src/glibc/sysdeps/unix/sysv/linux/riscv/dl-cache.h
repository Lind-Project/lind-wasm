...
#define add_system_dir(dir)                                                   \
  do                                                                          \
	...
	"/lib64/lp64d", "/lib64/lp64", "/lib32/ilp32d", "/lib32/ilp32", NULL, \
      };                                                                      \
	...
	{                                                                     \
      ...
      add_dir (path);                                                         \
      if (len >= lib_len && !memcmp (path + len - lib_len, "/lib", lib_len))  \
	for (ptr = lib_dirs; *ptr != NULL; ptr++)                             \
	  {                                                                   \
	    const char *lib_dir = *ptr;                                       \
	    size_t dir_len = strlen (lib_dir);                                \
	    ...
	    memcpy (path + len, lib_dir + lib_len, dir_len - lib_len + 1);    \
	    add_dir (path);                                                   \
	  }                                                                   \
    }                                                                         \
  while (0)
