...
#define CLEAR_STATE_COMMON state->__count = 0

#define BODY_FROM_HW(ASM)                                                     \
  {                                                                           \
    ASM;                                                                      \
    if (__glibc_likely (inptr == inend) || result == __GCONV_FULL_OUTPUT)     \
      break;                                                                  \
                                                                              \
    int i;                                                                    \
	...
	break;                                                                \
                                                                              \
    if (__glibc_likely (inptr + i == inend && result == __GCONV_EMPTY_INPUT)) \
      {                                                                       \
...
#  define BODY_FROM_C                                                         \
	...
	{                                                                     \
	  ...
	  if (ch >= 0xc2 && ch < 0xe0)                                        \
	    ...
	    {                                                                 \
		 ...
		 is the next byte with (x & 0xc0) != 0x80.  */                \
	      ...
	      do                                                              \
		++i;                                                          \
	      while (inptr + i < inend && (*(inptr + i) & 0xc0) == 0x80       \
		     && i < 5);                                               \
                                                                              \
	    errout:                                                           \
	      STANDARD_FROM_LOOP_ERR_HANDLER (i);                             \
	  ...
	  if (i < cnt                                                         \
	      || (cnt > 2                                                     \
		  && (ch >> (5 * cnt - 4))                                    \
			 == 0) /* Do not accept UTF-16 surrogates.  */        \
...
#  define BODY_TO_VX                                                                \
    {                                                                               \
	  ...
	  "    slgr %[R_INLEN],%[R_TMP2]\n\t" /* Calculate remaining uint32_t       \
						 values in loaded vrs.  */          \
	  "16: lghi %[R_TMP2],16\n\t"                                               \
	  "    sgr %[R_TMP2],%[R_TMP]\n\t"                                          \
	  "    l %[R_TMP],0(%[R_IN])\n\t"                                           \
	  "    aghi %[R_INLEN],-4\n\t"                                              \
	  ...
	  "    la %[R_OUT],1(%[R_OUT])\n\t"                                         \
	  "    brctg %[R_TMP2],20b\n\t"                                             \
	  "    j 0b\n\t" /* Switch to vx-loop.  */ /* Test if ch is 2byte           \
						      UTF-8 char. */                \
	  "22: clfi %[R_TMP],0x7ff\n\t"                                             \
	  ...
	  "    la %[R_OUT],2(%[R_OUT])\n\t"                                         \
	  "    brctg %[R_TMP2],20b\n\t"                                             \
	  "    j 0b\n\t" /* Switch to vx-loop.  */ /* Test if ch is 3-byte          \
						      UTF-8 char.  */               \
	  ...
	  "    la %[R_OUT],3(%[R_OUT])\n\t"                                         \
	  "    brctg %[R_TMP2],20b\n\t"                                             \
	  "    j 0b\n\t" /* Switch to vx-loop.  */ /* Test if ch is 4-byte          \
						      UTF-8 char.  */               \
	  "24: clfi %[R_TMP],0x10ffff\n\t"                                          \
	  "    jh 91f\n\t" /* ch > 0x10ffff is not allowed!  */ /* Handle           \
								   4-byte           \
								   UTF-8            \
								   char.  */        \
	  "34: slgfi %[R_OUTLEN],4\n\t"                                             \
	  ...
	  : /* outputs */[R_IN] "+a"(inptr), [R_INLEN] "+d"(inlen),                 \
	    ...
	    [R_TMP2] "=a"(tmp2), [R_TMP3] "=d"(tmp3), [R_RES] "+d"(result)          \
	  : /* inputs */                                                            \
	  [RES_OUT_FULL] "i"(__GCONV_FULL_OUTPUT),                                  \
	  [RES_IN_ILL] "i"(__GCONV_ILLEGAL_INPUT),                                  \
...
#  define BODY_TO_VX_CU                                                       \
    {                                                                         \
	  ...
	  "    sllg %[R_I],%[R_I],2\n\t"                                      \
	  "    la %[R_IN],0(%[R_I],%[R_IN])\n\t"                              \
	  "    slgr %[R_INLEN],%[R_I]\n\t" /* Handle multibyte utf8-char with \
					      convert instruction. */         \
	  "20: cu41 %[R_OUT],%[R_IN]\n\t"                                     \
	  "    jo 0b\n\t" /* Try vector implementation again.  */             \
	  "    lochil %[R_RES],%[RES_OUT_FULL]\n\t" /* cc == 1.  */           \
	  ...
	  : /* outputs */[R_IN] "+a"(pInput), [R_INLEN] "+d"(inlen),          \
	    ...
	    [R_TMP] "=d"(tmp), [R_I] "=a"(tmp2), [R_RES] "+d"(result)         \
	  : /* inputs */                                                      \
