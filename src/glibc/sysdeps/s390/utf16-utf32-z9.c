...
#define PREPARE_LOOP                                                          \
  ...
  int emit_bom = ((struct utf16_data *) step->__data)->emit_bom;              \
                                                                              \
  if (emit_bom && !data->__internal_use && data->__invocation_counter == 0)   \
    {                                                                         \
      if (dir == to_utf16)                                                    \
	{                                                                     \
...
#  define BODY_FROM_VX                                                          \
    {                                                                           \
	  ...
	  "    vlm %%v30,%%v31,0(%[R_TMP])\n\t" CONVERT_32BIT_SIZE_T (          \
	      [R_INLEN])                                                        \
	      CONVERT_32BIT_SIZE_T ([R_OUTLEN]) /* Loop which handles UTF-16    \
						   chars <0xd800, >0xdfff. */                                                                             \
	  ...
	  "    jl 99f\n\t" /* End if no bytes available.  */ /* Calculate       \
								remaining       \
								uint16_t        \
								values in       \
								inptr.  */      \
	  "    srlg %[R_TMP2],%[R_INLEN],1\n\t" /* Handle remaining uint16_t    \
						   values.  */                  \
	  "13: llh %[R_TMP],0(%[R_IN])\n\t"                                     \
	  "    slgfi %[R_OUTLEN],4\n\t"                                         \
	  "    jl 96f \n\t"                                                     \
	  ...
	  "    la %[R_OUT],4(%[R_OUT])\n\t"                                     \
	  "    brctg %[R_TMP2],13b\n\t"                                         \
	  "    j 0b\n\t" /* Switch to vx-loop.  */ /* Handle UTF-16 surrogate   \
						      pair.  */                 \
	  ...
	  "99:\n\t"                                                             \
	  ".machine pop"                                                        \
	  : /* outputs */[R_IN] "+a"(inptr), [R_INLEN] "+d"(inlen),             \
	    [R_OUT] "+a"(outptr), [R_OUTLEN] "+d"(outlen), [R_TMP] "=a"(tmp),   \
	    [R_TMP2] "=d"(tmp2), [R_TMP3] "=a"(tmp3), [R_RES] "+d"(result)      \
	  : /* inputs */                                                        \
	  [RES_OUT_FULL] "i"(__GCONV_FULL_OUTPUT),                              \
	  [RES_IN_ILL] "i"(__GCONV_ILLEGAL_INPUT),                              \
	  [RES_IN_FULL] "i"(__GCONV_INCOMPLETE_INPUT)                           \
	  : /* clobber list */ "memory",                                        \
	    "cc" ASM_CLOBBER_VR ("v16") ASM_CLOBBER_VR ("v17")                  \
		ASM_CLOBBER_VR ("v18") ASM_CLOBBER_VR ("v19")                   \
		    ASM_CLOBBER_VR ("v30") ASM_CLOBBER_VR ("v31"));             \
      if (__glibc_likely (inptr == inend) || result != __GCONV_ILLEGAL_INPUT)   \
	break;                                                                  \
                                                                                \
      STANDARD_FROM_LOOP_ERR_HANDLER (2);                                       \
    }
...
#  define BODY_FROM_VX_CU                                                       \
    {                                                                           \
	  ...
	  "    slgr %[R_INLEN],%[R_TMP]\n\t"                                    \
	  "    la %[R_OUT],0(%[R_TMP3],%[R_OUT])\n\t"                           \
	  "    slgr %[R_OUTLEN],%[R_TMP3]\n\t" /* Handles UTF16 surrogates      \
						  with convert instruction.     \
						*/                              \
	  ...
	  ".machine pop"                                                        \
	  : /* outputs */[R_IN] "+a"(pInput), [R_INLEN] "+d"(inlen),            \
	    [R_OUT] "+a"(pOutput), [R_OUTLEN] "+d"(outlen),                     \
	    [R_TMP] "=a"(tmp), [R_TMP2] "=d"(tmp2), [R_TMP3] "=a"(tmp3),        \
	    [R_RES] "+d"(result)                                                \
	  : /* inputs */                                                        \
	  [RES_OUT_FULL] "i"(__GCONV_FULL_OUTPUT), [RES_IN_ILL] "i"(            \
      ...
      outptr = pOutput;                                                         \
                                                                                \
      if (__glibc_likely (inlen == 0) || result == __GCONV_FULL_OUTPUT)         \
	break;                                                                  \
      if (inlen == 1)                                                           \
	{                                                                       \
...
#  define BODY_TO_C                                                           \
    {                                                                         \
      uint32_t c = get32 (inptr);                                             \
                                                                              \
      if (__builtin_expect (c <= 0xd7ff, 1) || (c > 0xdfff && c <= 0xffff))   \
	{                                                                     \
	  /* Two UTF-16 chars.  */                                            \
	  put16 (outptr, c);                                                  \
	}                                                                     \
...
#  define BODY_TO_VX                                                                  \
    {                                                                                 \
	  ...
	  "    vlm %%v30,%%v31,0(%[R_TMP])\n\t" CONVERT_32BIT_SIZE_T (                \
	      [R_INLEN])                                                              \
	      CONVERT_32BIT_SIZE_T ([R_OUTLEN]) /* Loop which handles UTF-32          \
						   chars ch < 0xd800 || (ch >                                                                 \
						   0xdfff && ch < 0x10000).           \
						 */                                   \
	  ...
	  "    la %[R_OUT],0(%[R_TMP3],%[R_OUT])\n\t"                                 \
	  "    slgr %[R_OUTLEN],%[R_TMP3]\n\t" /* Calculate remaining                 \
						  uint32_t values in vrs.  */         \
	  "12: lghi %[R_TMP2],8\n\t"                                                  \
	  "    srlg %[R_TMP3],%[R_TMP3],1\n\t"                                        \
	  "    slgr %[R_TMP2],%[R_TMP3]\n\t" /* Handle remaining UTF-32               \
						characters.  */                       \
	  "20: l %[R_TMP],0(%[R_IN])\n\t"                                             \
	  ...
	  "    la %[R_OUT],2(%[R_OUT])\n\t"                                           \
	  "    brctg %[R_TMP2],20b\n\t"                                               \
	  "    j 0b\n\t" /* Switch to vx-loop.  */ /* Test if ch is 4byte             \
						      UTF-16 char. */                 \
	  "21: clfi %[R_TMP],0x10ffff\n\t"                                            \
	  "    jh 91f\n\t" /* ch > 0x10ffff is not allowed!  */ /* Handle 4           \
								   byte UTF16         \
								   char.  */          \
	  "    slgfi %[R_OUTLEN],4\n\t"                                               \
	  "    jl 90f \n\t"                                                           \
...
#  define BODY_TO_VX_CU                                                       \
    {                                                                         \
      ...
      asm volatile (".machine push\n\t"                                       \
			...
			[R_INLEN])                                            \
			CONVERT_32BIT_SIZE_T (                                \
			    [R_OUTLEN]) /* Loop which handles UTF-32 chars    \
		    ...
		    "    clgijl %[R_OUTLEN],16,20f\n\t"                       \
		    "    j 1b\n\t" /* Setup to check for ch >= 0xd800 && ch   \
				      <= 0xdfff and check for ch >= 0x10000.                                                 \
				      (v30, v31)  */                          \
		    "9:  .long 0xd800,0xdfff,0x10000,0x10000\n\t"             \
		    "    .long 0xa0000000,0xc0000000, "                       \
		    "0xa0000000,0xa0000000\n\t" /* At least one UTF32 char is \
						   in range of surrogates.    \
						   Store the preceding        \
						   characters.  */            \
		    "11: ahi %[R_TMP2],16\n\t"                                \
		    "10: vlgvb %[R_TMP],%%v19,7\n\t"                          \
