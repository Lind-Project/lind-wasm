...
#    define ICONV_VX_IFUNC(FUNC)                                              \
      s390_libc_ifunc_expr (ICONV_C_NAME (FUNC), FUNC,                        \
			    (hwcap & HWCAP_S390_VX) ? ICONV_VX_NAME (FUNC)    \
						    : ICONV_C_NAME (FUNC))
#  endif
...
#  define BODY                                                                                                                                                           \
	...
	len = (outend - outptr) / 4;                                                                                                                                     \
	  ...
	  ".machinemode \"zarch_nohighgprs\"\n\t" CONVERT_32BIT_SIZE_T ([R_LEN]) "    vrepib %%v30,0x7f\n\t" /* For compare > 0x7f.  */                                  \
										 ...
										 "    lghi %[R_LI],15\n\t"                                                               \
										 "    ngr %[R_LEN],%[R_LI]\n\t"                                                          \
										 "    je 20f\n\t" /* Jump away if no remaining bytes.  */ /* Handle remaining bytes.  */ \
										 ...
										 "20:\n\t"                                                                               \
										 ".machine pop"                                                                          \
	  : /* outputs */[R_OUT] "+a"(outptr), [R_IN] "+a"(inptr),                                                                                                       \
	    [R_LEN] "+d"(len), [R_LI] "=d"(loop_count), [R_TMP] "=a"(tmp)                                                                                                \
	  : /* inputs */                                                                                                                                                 \
	  : /* clobber list*/ "memory",                                                                                                                                  \
...
#  define BODY                                                                  \
	...
	len = outend - outptr;                                                  \
	  ...
	  ".machinemode \"zarch_nohighgprs\"\n\t" CONVERT_32BIT_SIZE_T (        \
	      [R_LEN]) /* Setup to check for ch > 0x7f.  */                     \
	  "    vzero %%v21\n\t"                                                 \
	  "    srlg %[R_LI],%[R_LEN],4\n\t"                                     \
	  "    vleih %%v21,8192,0\n\t"	/* element 0:   >  */                   \
	  "    vleih %%v21,-8192,2\n\t" /* element 1: =<>  */                   \
	  ...
	  "    lghi %[R_LI],15\n\t"                                             \
	  "    ngr %[R_LEN],%[R_LI]\n\t"                                        \
	  "    je 20f\n\t" /* Jump away if no remaining bytes.  */ /* Handle    \
								      remaining \
								      bytes.    \
								    */          \
	  "1: sllg %[R_LI],%[R_LEN],2\n\t"                                      \
	  "    aghi %[R_LI],-1\n\t"                                             \
	  "    jl 20f\n\t" /* Jump away if no remaining bytes.  */ /* Load      \
								      remaining \
								      1...63    \
								      bytes.    \
								    */          \
	  ...
	  ".machine pop"                                                        \
	  : /* outputs */[R_OUT] "+a"(outptr), [R_IN] "+a"(inptr),              \
	    [R_LEN] "+d"(len), [R_LI] "=d"(loop_count), [R_I] "=a"(tmp2),       \
	    [R_TMP] "=d"(tmp)                                                   \
	  : /* inputs */                                                        \
	  : /* clobber list*/ "memory",                                         \
	    "cc" ASM_CLOBBER_VR ("v16") ASM_CLOBBER_VR ("v17")                  \
...
ICONV_VX_NAME (internal_ucs4le_loop) (
    struct __gconv_step *step, struct __gconv_step_data *step_data,
    const unsigned char **inptrp, const unsigned char *inend,
    unsigned char **outptrp, const unsigned char *outend, size_t *irreversible)
{
...
static inline int __attribute ((always_inline))
ICONV_VX_NAME (ucs4_internal_loop) (
    struct __gconv_step *step, struct __gconv_step_data *step_data,
    const unsigned char **inptrp, const unsigned char *inend,
    unsigned char **outptrp, const unsigned char *outend, size_t *irreversible)
{
  ...
  do
    {
	  ...
	  ".machine pop"
	  : /* outputs */[R_OUT] "+a"(outptr), [R_IN] "+a"(inptr),
	    [R_LI] "=a"(loop_count), [R_LEN] "+d"(len)
	  : /* inputs */
	  : /* clobber list*/ "memory",
	    "cc" ASM_CLOBBER_VR ("v16") ASM_CLOBBER_VR ("v20")
...
static inline int __attribute ((always_inline))
ICONV_VX_NAME (ucs4le_internal_loop) (
    struct __gconv_step *step, struct __gconv_step_data *step_data,
    const unsigned char **inptrp, const unsigned char *inend,
    unsigned char **outptrp, const unsigned char *outend, size_t *irreversible)
{
...
#  define BODY                                                                    \
    {                                                                             \
	  ...
	  ".machine \"z13\"\n\t"                                                  \
	  ".machinemode \"zarch_nohighgprs\"\n\t" CONVERT_32BIT_SIZE_T (          \
	      [R_LEN]) /* Setup to check for ch >= 0xd800 && ch < 0xe000.  */     \
	  "    larl %[R_TMP],9f\n\t"                                              \
	  "    vlm %%v20,%%v21,0(%[R_TMP])\n\t"                                   \
	  "    srlg %[R_TMP],%[R_LEN],3\n\t"                                      \
	  ...
	  ".machine pop"                                                          \
	  : /* outputs */[R_OUT] "+a"(outptr), [R_IN] "+a"(inptr),                \
	    [R_TMP] "=a"(tmp), [R_TMP2] "=a"(tmp2), [R_LEN] "+d"(len)             \
	  : /* inputs */                                                          \
	  : /* clobber list*/ "memory",                                           \
	    "cc" ASM_CLOBBER_VR ("v16") ASM_CLOBBER_VR ("v17")                    \
		ASM_CLOBBER_VR ("v18") ASM_CLOBBER_VR ("v19")                     \
...
#  define TO_LOOP                                                             \
    ICONV_VX_NAME (ucs2reverse_internal_loop) /* This is not used.*/
...
#  define BODY                                                                       \
    {                                                                                \
      if (__builtin_expect (inend - inptr < 32, 1) || outend - outptr < 16)          \
	  ...
	  __asm__ volatile (                                                         \
		  ...
		  [R_LI]) "    larl %[R_I],3f\n\t"                                   \
			  ...
			  "    jo 2b\n\t" /* All ch's in this range, proceed.        \
					   */                                        \
			  "    lghi %[R_TMP],16\n\t"                                 \
			  "    j 12f\n\t" /* Maybe v16 contains invalid              \
					     chars. Check ch >= 0xe000 && ch                                                            \
					     <= 0xffff.  */                          \
			  ...
			  ".machine pop"                                             \
	      : /* outputs */[R_OUT] "+a"(outptr), [R_IN] "+a"(inptr),               \
		[R_LI] "+d"(loop_count), [R_I] "=a"(tmp2), [R_TMP] "=d"(tmp)         \
	      : /* inputs */                                                         \
	      : /* clobber list*/ "memory",                                          \
		"cc" ASM_CLOBBER_VR ("v16") ASM_CLOBBER_VR ("v17")                   \
		    ASM_CLOBBER_VR ("v18") ASM_CLOBBER_VR ("v19")                    \
...
#  define TO_LOOP                                                             \
    ICONV_VX_NAME (internal_ucs2reverse_loop) /* This is not used.*/
