...
#  define BODY                                                                \
    {                                                                         \
      if (__builtin_expect (inend - inptr < 16, 1) || outend - outptr < 64)   \
	...
	{                                                                     \
	  ...
	  __asm__ volatile (                                                  \
	      ...
	      "    vl %%v24,0(%[R_I],%[R_IN])\n\t"                            \
	      "    vceqb %%v24,%%v0,%%v24\n\t" /* Zeros in buf (v23) and      \
						  inptr (v24) are marked with                                                                         \
						  one bits. After xor,        \
						  invalid characters are                                                                          \
						  marked as one bits.         \
						  Proceed, if no invalid                                                                      \
	      ...
	      "20: \n\t"                                                      \
	      ".machine pop"                                                  \
	      : /* outputs */[R_IN] "+a"(inptr), [R_OUT] "+a"(outptr),        \
		[R_I] "=&a"(index), [R_LI] "+a"(loop_count)                   \
	      : /* inputs */[R_BUF] "a"(buf), [R_TBL] "a"(to_ucs1)            \
	      : /* clobber list*/ "memory",                                   \
		"cc" ASM_CLOBBER_VR ("v0") ASM_CLOBBER_VR ("v16")             \
...
#  define BODY                                                                     \
    {                                                                              \
      if (__builtin_expect (inend - inptr < 64, 1) || outend - outptr < 16)        \
	/* Convert remaining bytes with c code.  */                                \
	BODY_TO_ORIG                                                               \
      else                                                                         \
	{                                                                          \
	  ...
	  __asm__ volatile (                                                       \
	      ...
	      "20: \n\t"                                                           \
	      ".machine pop"                                                       \
	      : /* outputs */[R_IN] "+a"(inptr), [R_I] "=&a"(index),               \
		[R_TMP] "=d"(tmp), [R_LI] "+d"(remaining_loop_count)               \
	      : /* inputs */[R_BUF] "a"(buf), [R_MAX] "d"(max)                     \
	      : /* clobber list*/ "memory",                                        \
	  ...
	  if (loop_count > 0)                                                      \
	    {                                                                      \
		 ...
		 check for errors (from_ucs4[ch] == 0).  */                        \
		  ...
		  "20: \n\t"                                                       \
		  ".machine pop"                                                   \
		  : /* outputs */[R_OUT] "+a"(outptr), [R_I] "=&a"(tmp),           \
		    [R_LI] "+d"(loop_count)                                        \
		  : /* inputs */[R_BUF] "a"(buf), [R_TBL] "a"(from_ucs4)           \
		  : /* clobber list*/ "memory",                                    \
		    "cc" ASM_CLOBBER_VR ("v16") ASM_CLOBBER_VR ("v17"));           \
