...
#define elf_machine_type_class(type)                                          \
  ((((type) == R_AARCH64_JUMP_SLOT || (type) == R_AARCH64_TLS_DTPMOD          \
     || (type) == R_AARCH64_TLS_DTPREL || (type) == R_AARCH64_TLS_TPREL       \
  ...
  if (__builtin_expect (r_type == AARCH64_R (JUMP_SLOT), 1))
      ...
      if (__glibc_unlikely (map->l_info[DT_AARCH64 (VARIANT_PCS)] != NULL))
	{
	  ...
	  if (__glibc_unlikely (sym->st_other & STO_AARCH64_VARIANT_PCS))
		...
		{
		  const ElfW (Half) *vernum = (const void *) D_PTR (
		      map, l_info[VERSYMIDX (DT_VERSYM)]);
