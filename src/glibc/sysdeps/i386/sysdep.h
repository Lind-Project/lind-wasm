...
#  define SETUP_PIC_REG(reg)                                                  \
    ...
    .section.text.GET_PC_THUNK (reg), "axG", @progbits, GET_PC_THUNK (reg),   \
	comdat;                                                               \
    .globl GET_PC_THUNK (reg);                                                \
    .hidden GET_PC_THUNK (reg);                                               \
    .p2align 4;                                                               \
    .type GET_PC_THUNK (reg), @function;                                      \
    GET_PC_THUNK (reg) : movl (% esp), % e##reg;                              \
    ret;                                                                      \
...
#  define SETUP_PIC_REG_STR(reg)                                                                                                 \
    ".ifndef " GET_PC_THUNK_STR (reg) "\n"                                                                                       \
				      ".section .text." GET_PC_THUNK_STR (reg) ",\"axG\",@progbits," GET_PC_THUNK_STR (          \
					  reg) ",comdat\n"                                                                       \
					       ".globl " GET_PC_THUNK_STR (                                                      \
						   reg) "\n"                                                                     \
							".hidden"                                                                \
							" " GET_PC_THUNK_STR (                                                   \
							    reg) "\n"                                                            \
								 ...
								 " " GET_PC_THUNK_STR (reg) ",@function\n" GET_PC_THUNK_STR (    \
								     reg) ":"                                                    \
									  ...
									  "z"                                                    \
									  "e"                                                    \
									  " " GET_PC_THUNK_STR (reg) ", . - " GET_PC_THUNK_STR ( \
									      reg) "\n"                                          \
										   ".previous\n"                                 \
										   ".endif\n"                                    \
										   "call " GET_PC_THUNK_STR (                    \
										       reg)
