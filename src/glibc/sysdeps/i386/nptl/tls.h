...
#      define TLS_GET_GS()                                                    \
	({                                                                    \
	  int __seg = 0; /* Assuming __seg can be retrieved in some           \
...
tls_fill_user_desc (union user_desc_init *desc, unsigned int entry_number,
		    void *pd)
{
...
#    define TLS_DEFINE_INIT_TP(tp, pd)                                        \
      union user_desc_init _segdescr;                                         \
      /* Find the 'entry_number' field that the kernel selected in            \
	 TLS_INIT_TP. The first three bits of the segment register value                           \
	 select the GDT, ignore them.  We get the index from the value of the                         \
	 %gs register in the current thread.  */                                                      \
...
#    define THREAD_GSCOPE_RESET_FLAG()                                        \
      do                                                                      \
	{                                                                     \
	  int __res = atomic_exchange_explicit (                              \
