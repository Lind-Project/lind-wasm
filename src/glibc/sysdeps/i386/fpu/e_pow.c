...
double __ieee754_pow(double x, double y) {
    ...
    if (isinf(x)) {
        ...
        if (fmod(y, 2.0) == 0.0) {
            ...
            return copysign(HUGE_VAL, x);  // pow(inf, positive odd integer) follows the sign of x
        }
    }
    ...
    if (x < 0.0) {
        ...
        if (y != floor(y)) {
            errno = EDOM;  // Domain error for negative base with non-integer exponent
            return NAN;
        }
        // Calculate power for negative base with integer exponent
        double result = exp(y * log(fabs(x)));
        if (fmod(y, 2.0) == 1.0) {  // Adjust sign if exponent is odd
            return -result;
        }
        return result;
    }
