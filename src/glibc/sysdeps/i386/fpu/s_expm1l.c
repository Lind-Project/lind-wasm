...
long double
ieee754_expl_1l (long double x, long double base_log2)
{
  ...
  long double fract_part = modfl (
      x_log2_base, &int_part); // Separate integer and fractional parts
...
{
  // expm1(x) = exp(x) - 1, adjust for this
  long double result = ieee754_expl_1l (x, M_LOG2E);
  return result - 1.0L;
}
libm_hidden_def (__expm1l) libm_alias_finite (__expm1, expm1)
