...
/**
 * Multiply a limb vector with a limb and add the result to a second limb
 * vector.
 ...
 */
uint32_t
__mpn_addmul_1 (uint32_t *res_ptr, const uint32_t *s1_ptr, size_t size,
		uint32_t s2_limb)
{
  uint32_t carry = 0;
  ...
  for (size_t i = 0; i < size; i++)
    {
      ...
      if (sum < res_ptr[i] || sum < low_product)
	carry = 1;
      else
	carry = 0;
  ...
  // Return the final carry value, which may need to be added to the next limb
