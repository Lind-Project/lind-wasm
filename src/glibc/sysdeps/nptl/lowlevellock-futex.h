...
#    define lll_futex_syscall(nargs, futexp, op, ...)                         \
      ({                                                                      \
	long int __ret = MAKE_RAW_SYSCALL##nargs (                            \
	    FUTEX_SYSCALL, "syscall|futex", futexp, op, __VA_ARGS__);         \
...
#    define lll_futex_timed_wait(futexp, val, timeout, private)               \
      lll_futex_syscall (4, futexp, __lll_private_flag (FUTEX_WAIT, private), \
			 val, timeout)
...
#    define lll_futex_wake(futexp, nr, private)                               \
      lll_futex_syscall (4, futexp, __lll_private_flag (FUTEX_WAKE, private), \
			 nr, 0)
...
#    define lll_futex_wake_unlock(futexp, nr_wake, nr_wake2, futexp2,         \
				  private)                                    \
      lll_futex_syscall (                                                     \
	  6, futexp, __lll_private_flag (FUTEX_WAKE_OP, private), nr_wake,    \
	  nr_wake2, futexp2, FUTEX_OP_CLEAR_WAKE_IF_GT_ONE)

#    define lll_futex_timed_unlock_pi(futexp, private)                        \
      lll_futex_syscall (4, futexp,                                           \
			 __lll_private_flag (FUTEX_UNLOCK_PI, private), 0, 0)
...
#    define lll_futex_timed_wait_cancel(futexp, val, timeout, private)        \
	...
	int __oldtype = LIBC_CANCEL_ASYNC ();                                 \
	long int __err                                                        \
	    = lll_futex_timed_wait (futexp, val, timeout, private);           \
	LIBC_CANCEL_RESET (__oldtype);                                        \
	__err;                                                                \
      })
