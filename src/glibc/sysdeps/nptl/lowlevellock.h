...
libc_hidden_proto (__lll_lock_wait_private) extern void __lll_lock_wait (
    int *futex, int private);
...
libc_hidden_proto (__lll_lock_wake_private) extern void __lll_lock_wake (
    int *futex, int private) __THROW;
...
libc_hidden_proto (__lll_clocklock_elision)
    ...
    extern int __lll_lock_elision (int *futex, short *adapt_count,
				   int private);
libc_hidden_proto (__lll_lock_elision)

#    if ELISION_UNLOCK_NEEDS_ADAPT_COUNT
    extern int __lll_unlock_elision (int *lock, short *adapt_count,
				     int private);
#    else
...
#    define lll_clocklock_elision(futex, adapt_count, clockid, timeout,       \
				  private)                                    \
      __lll_clocklock_elision (&(futex), &(adapt_count), clockid, timeout,    \
			       private)
...
#    define lll_lock_elision(lock, try_lock, private)                         \
      ({                                                                      \
	lll_lock (lock, private);                                             \
	0;                                                                    \
      })
#    define lll_trylock_elision(a, t) lll_trylock (a)
#    define lll_unlock_elision(a, b, c)                                       \
      ({                                                                      \
	lll_unlock (a, c);                                                    \
	0;                                                                    \
      })
