...
#    define __TEST_AND_BEGIN_NON_STOP(old, new)                               \
      do                                                                      \
	{                                                                     \
	}                                                                     \
      while ((old) * (new) * 0 != 0)
#    define __TEST_AND_END_NON_STOP(old, new)                                 \
      do                                                                      \
	{                                                                     \
	}                                                                     \
      while ((old) * (new) * 0 != 0)
		       ...
		       unsigned long long new_mask)
{
  ...
  /* If requesting to keep status, replace control, and merge exceptions,
     ...
     of the whole FPSCR.  */
  if ((old_mask
       & (FPSCR_CONTROL_MASK | FPSCR_STATUS_MASK | FPSCR_EXCEPTIONS_MASK))
...
{
  return __libc_femergeenv_ppc (envp, ~FPSCR_CONTROL_MASK, ~FPSCR_STATUS_MASK)
	 & ex;
}
