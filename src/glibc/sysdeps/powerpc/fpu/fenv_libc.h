...
#  define __TEST_AND_ENTER_NON_STOP(old, new)                                 \
    do                                                                        \
      {                                                                       \
	if (((old) & FPSCR_ENABLES_MASK) != 0                                 \
	    && ((new) & FPSCR_ENABLES_MASK) == 0)                             \
	  (void) __fe_mask_env ();                                            \
      }                                                                       \
    while (0)
...
#  define FPSCR_ENABLES_MASK                                                  \
    (FPSCR_VE_MASK | FPSCR_OE_MASK | FPSCR_UE_MASK | FPSCR_ZE_MASK            \
     | FPSCR_XE_MASK)
#  define FPSCR_BASIC_EXCEPTIONS_MASK                                         \
    (FPSCR_VX_MASK | FPSCR_OX_MASK | FPSCR_UX_MASK | FPSCR_ZX_MASK            \
     | FPSCR_XX_MASK)
#  define FPSCR_EXCEPTIONS_MASK                                               \
    (FPSCR_BASIC_EXCEPTIONS_MASK | FPSCR_VXSNAN_MASK | FPSCR_VXISI_MASK       \
     | FPSCR_VXIDI_MASK | FPSCR_VXZDZ_MASK | FPSCR_VXIMZ_MASK                 \
     | FPSCR_VXVC_MASK | FPSCR_VXSOFT_MASK | FPSCR_VXSQRT_MASK                \
     | FPSCR_VXCVI_MASK)
#  define FPSCR_FPRF_MASK                                                     \
    (FPSCR_FPRF_C_MASK | FPSCR_FPRF_FL_MASK | FPSCR_FPRF_FG_MASK              \
     | FPSCR_FPRF_FE_MASK | FPSCR_FPRF_FU_MASK)
#  define FPSCR_CONTROL_MASK                                                  \
    (FPSCR_ENABLES_MASK | FPSCR_NI_MASK | FPSCR_RN_MASK)
...
{
  return (unsigned long long) (excepts & FE_ALL_EXCEPT)
	 >> FPSCR_EXCEPT_TO_ENABLE_SHIFT;
}
...
#  define f_wash(x)                                                           \
    ({                                                                        \
      double d;                                                               \
      asm volatile ("fmul %0,%1,%2" : "=f"(d) : "f"(x), "f"((float) 1.0));    \
      d;                                                                      \
    })
#  define f_washf(x)                                                          \
    ({                                                                        \
      float f;                                                                \
      asm volatile ("fmuls %0,%1,%2" : "=f"(f) : "f"(x), "f"((float) 1.0));   \
      f;                                                                      \
    })
