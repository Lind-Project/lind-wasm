  ...
  if (a_val & PPC_FEATURE_HAS_DFP)
    {
      ...
      if (which == 0)
	di_fpscr = ((di_fpscr & _FPSCR_RESERVED)
		    | (_FPSCR_TEST0_DRN | _FPSCR_TEST0_RN));
      else
	di_fpscr = ((di_fpscr & _FPSCR_RESERVED)
		    | (_FPSCR_TEST1_DRN | _FPSCR_TEST1_RN));
...
test_special_regs (int which)
{
  ...
  if (a_val & PPC_FEATURE_HAS_DFP)
	...
	{
	  printf ("%s: DRN and RN bits set before getcontext were not "
		  "preserved across [set|swap]context call: %m",
		  __FUNCTION__);
	  _exit (3);
	}
    }
	...
	puts ("After swapcontext the 32-bit FPSCR contains:");
      ...
      if ((di_fpscr & test) != test)
	{
	  printf ("%s: RN bit set before getcontext was not preserved across "
		  "[set|swap]context call: %m",
		  __FUNCTION__);
	  _exit (4);
	}
    }
