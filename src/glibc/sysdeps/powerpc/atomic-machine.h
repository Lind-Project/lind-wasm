...
#define __arch_compare_and_exchange_val_32_acq(mem, newval, oldval)           \
  ({                                                                          \
    ...
    __asm __volatile ("1:	lwarx	%0,0,%1" MUTEX_HINT_ACQ "\n"          \
		      "	cmpw	%0,%2\n"                                         \
		      "	bne	2f\n"                                             \
		      "	stwcx.	%3,0,%1\n"                                     \
		      "	bne-	1b\n"                                            \
...
#define atomic_compare_and_exchange_val_rel(mem, newval, oldval)              \
  ({                                                                          \
    ...
    if (sizeof (*mem) == 4)                                                   \
      __result                                                                \
	  = __arch_compare_and_exchange_val_32_rel (mem, newval, oldval);     \
    else if (sizeof (*mem) == 8)                                              \
      __result                                                                \
	  = __arch_compare_and_exchange_val_64_rel (mem, newval, oldval);     \
    else                                                                      \
      abort ();                                                               \
    __result;                                                                 \
  })
