...
static const char *const rmnames[4] = { "nearest", "zero", "+Inf", "-Inf" };
...
static const optest_t optests[] = {
  ...
  { __LINE__, B_ADD, 1, P_Z, P_Z, 0, P_Z, P_Z, R_ALL & ~R_DOWN, 0, 0, P_Z,
    P_Z },
  ...
  { __LINE__, B_ADD, 0, P_O0, P_Z, 1, P_O0, P_Z, R_ALL & ~R_DOWN, 0, 0, P_Z,
    P_Z },
  { __LINE__, B_ADD, 0, P_O0, P_Z, 1, P_O0, P_Z, R_DOWN, 0, 1, P_Z, P_Z },
...
check_op (void)
{
  ...
  for (i = 0; i < sizeof (optests) / sizeof (optests[0]); i++)
    {
      a = pattern (optests[i].a_sgn, optests[i].a_exp, optests[i].a_mant);
      b = pattern (optests[i].b_sgn, optests[i].b_exp, optests[i].b_mant);
      x = pattern (optests[i].x_sgn, optests[i].x_exp, optests[i].x_mant);
      for (j = 0; j < 4; j++)
	if (optests[i].rmode & 1 << j)
	  {
	    ...
	    check_excepts (optests[i].line, rmnames[j], optests[i].excepts,
			   raised);
	  }
    }
}
