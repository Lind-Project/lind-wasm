...
#  ifdef __LITTLE_ENDIAN__
    (hwcap2 & PPC_FEATURE2_ARCH_3_1 && hwcap2 & PPC_FEATURE2_HAS_ISEL
     && hwcap & PPC_FEATURE_HAS_VSX && GLRO (dl_cache_line_size) == 128)
	? __memset_power10
    :
#  endif
	...
	? __memset_power8
    : (hwcap & PPC_FEATURE_ARCH_2_06 && GLRO (dl_cache_line_size) == 128)
	? __memset_power7
    : (hwcap & PPC_FEATURE_ARCH_2_05 && GLRO (dl_cache_line_size) == 128)
	? __memset_power6
    : (hwcap & PPC_FEATURE_POWER4 && GLRO (dl_cache_line_size) == 128)
	? __memset_power4
	: __memset_ppc);
