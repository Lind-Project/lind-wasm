...
.macro SAVE_ARG NARG.if \NARG SAVE_ARG \NARG - 1 std 2 +\NARG,
    -FRAME_MIN_SIZE_PARM + FRAME_PARM_SAVE - 8
	+ 8
	      * (\NARG) (1)
		    .endif
		    .endm

		    .macro REST_ARG NARG.if \NARG REST_ARG \NARG
	- 1 ld 2 +\NARG,
    FRAME_PARM_SAVE - 8
	+ 8
	      * (\NARG) (1)
		    ...
		    .macro CFI_SAVE_ARG NARG.if \NARG CFI_SAVE_ARG \NARG
	- 1 cfi_offset (2 +\NARG, -FRAME_MIN_SIZE_PARM + FRAME_PARM_SAVE - 8
				      + 8 * (\NARG))
	      .endif
	      .endm

	      .macro CFI_REST_ARG NARG.if \NARG CFI_REST_ARG \NARG
	- 1 cfi_restore (2 +\NARG)
	      .endif
	      .endm

	      /* If compiled for profiling, call `_mcount' at the start of each
		 function. see ppc-mcount.S for more details.  */
	      .macro CALL_MCOUNT NARG
#  ifdef PROF
	  mflr r0 SAVE_ARG \NARG std r0,
    FRAME_LR_SAVE (r1) stdu r1,
    -FRAME_MIN_SIZE_PARM (r1) cfi_adjust_cfa_offset (FRAME_MIN_SIZE_PARM)
	cfi_offset (lr, FRAME_LR_SAVE) CFI_SAVE_ARG \NARG bl
    JUMPTARGET (_mcount)
#    ifndef SHARED
	nop
#    endif
    ld r0,
    FRAME_MIN_SIZE_PARM + FRAME_LR_SAVE (r1) REST_ARG \NARG mtlr r0 addi r1, r1
    ,
    FRAME_MIN_SIZE_PARM cfi_adjust_cfa_offset (-FRAME_MIN_SIZE_PARM)
	cfi_restore (lr) CFI_REST_ARG \NARG
#  endif
	    .endm
...
#  if _CALL_ELF != 2
	    ...
	    .macro PPC64_LOAD_FUNCPTR PTR ld r12,
    0(\PTR) ld r2, 8(\PTR) mtctr r12 ld r11,
    16(\PTR).endm
...
#    define ENTRY_2(name)                                                     \
      .globl name;                                                            \
      .type name, @function
#    define END_2(name) .size name, .- name
#    define LOCALENTRY(name)                                                  \
...
#  endif /* _CALL_ELF */
	    ...
	    .macro NOPS NARG.if \NARG NOPS \NARG
	- 1 nop.endif
	      .endm
...
#    define ENTRY(name, ...) ENTRY_TOCLESS (name, ##__VA_ARGS__)
#  endif
...
/* We will allocate a new frame to save LR and the non-volatile register used
   to read the TCB when checking for scv support on syscall code.  We actually
   just need the minimum frame size plus room for 1 reg (8 bytes).  But the ABI
...
#  define NVOLREG_RESTORE                                                     \
    ...
    cfi_adjust_cfa_offset (-SCV_FRAME_SIZE);
	  ...
	  /* Check PPC_FEATURE2_SCV bit from hwcap2 in the TCB.  If it is not
	     set, scv is not available, then go to JUMPFALSE (label given by
	     the macro's caller).  We save the value we read from the TCB in a
	     non-volatile register so we can reuse it later when exiting from
	     the syscall in PSEUDO_RET.  Note that for the static case we need
	     an extra check to guarantee the thread pointer has already been
	     initialized, otherwise we may try to access an invalid address if
	     a syscall is called before the TLS has been setup.  */
...
#  endif
    ...
    ld \REG,
    TCB_HWCAP (PT_THREAD_POINTER) andis. \REG,\REG,
    PPC_FEATURE2_SCV >> 16
...
#  ifdef SHARED
#    define TAIL_CALL_SYSCALL_ERROR b JUMPTARGET (NOTOC (__syscall_error))
#  else
...
#      define __GLRO_DEF(var)                                                 \
	.LC__##var :.tc _rtld_local_ro[TC], _rtld_local_ro
#    else
#      define __GLRO_DEF(var)                                                 \
	.LC__##var :.tc _rtld_global_ro[TC], _rtld_global_ro
#    endif
...
#  else
#    define __GLRO_DEF(var) .LC__##var :.tc _##var[TC], _##var
#    define __GLRO(rOUT, var, offset)                                         \
      addis rOUT, r2, .LC__##var @toc @ha;                                    \
      ld rOUT, .LC__##var @toc @l (rOUT);                                     \
      lwz rOUT, 0(rOUT)
