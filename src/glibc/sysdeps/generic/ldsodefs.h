...
_dl_higher_prime_number (unsigned long int n) attribute_hidden;
...
#  else
	__attribute__ ((always_inline)) static inline void _dl_signal_cexception (
	    int errcode, struct dl_exception *exception, const char *occasion)
    {
...
#  else
    ...
    _dl_signal_cerror (int errcode, const char *object, const char *occasion,
		       const char *errstring)
    {
...
/* Restricted version of _dl_lookup_symbol_x.  Searches MAP (and only
   ...
   undefined.  */
const ElfW (Sym)
    * _dl_lookup_direct (struct link_map *map, const char *undef_name,
			 uint32_t new_hash, const char *version,
...
_dl_important_hwcaps (const char *prepend, const char *mask, size_t *sz,
...
void _dl_audit_symbind (struct link_map *l, struct reloc_result *reloc_result,
			...
			DL_FIXUP_VALUE_TYPE *value, lookup_t result,
			bool lazy) attribute_hidden;
...
void DL_ARCH_FIXUP_ATTRIBUTE
_dl_audit_pltexit (struct link_map *l, ElfW (Word) reloc_arg,
		   const void *inregs, void *outregs) attribute_hidden;
