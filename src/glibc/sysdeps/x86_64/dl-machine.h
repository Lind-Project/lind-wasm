  ...
  if (l->l_info[DT_JMPREL] && lazy)
      ...
      if (__glibc_unlikely (profile))
	{
	  ...
	  if (X86_ISA_CPU_FEATURE_USABLE_P (cpu_features, AVX512F))
	    *(ElfW (Addr) *) (got + 2)
		= (ElfW (Addr)) & _dl_runtime_profile_avx512;
	  else if (X86_ISA_CPU_FEATURE_USABLE_P (cpu_features, AVX))
	    *(ElfW (Addr) *) (got + 2)
		= (ElfW (Addr)) & _dl_runtime_profile_avx;
	  else
	    *(ElfW (Addr) *) (got + 2)
		= (ElfW (Addr)) & _dl_runtime_profile_sse;
  ...
  if (__glibc_unlikely (r_type == R_X86_64_RELATIVE))
    *reloc_addr = map->l_addr + reloc->r_addend;
	...
	if (sym != NULL
	  ...
	  {
#ifndef RTLD_BOOTSTRAP
	    if (sym_map != map && !sym_map->l_relocated)
	      {
		const char *strtab
...
%s: Relink `%s' with `%s' for IFUNC symbol `%s'\n",
				    ...
				    sym_map->l_name, strtab + refsym->st_name);
	      }
#endif
...
x86_64_rewrite_plt (struct link_map *map, ElfW (Addr) plt_rewrite)
{
  ...
  for (; reloc < reloc_end; reloc++)
	...
	if (((uint64_t) disp + (uint64_t) ((uint32_t) INT32_MIN))
	  ...
	  {
	    if (GL (dl_x86_feature_control).plt_rewrite != plt_rewrite_jmpabs)
	      {
...
x86_64_rewrite_plt_in_place (struct link_map *map)
{
  ...
  ElfW (Addr) plt = (map->l_info[DT_X86_64 (PLT)]->d_un.d_ptr + map->l_addr);
  size_t pagesize = GLRO (dl_pagesize);
  ElfW (Addr) plt_aligned = ALIGN_DOWN (plt, pagesize);
  size_t pltsz
      = (map->l_info[DT_X86_64 (PLTSZ)]->d_un.d_val + plt - plt_aligned);
  ...
  if (__glibc_unlikely (
	  __mprotect ((void *) plt_aligned, pltsz, PROT_WRITE | PROT_READ)
	  < 0))
    {
      if (__glibc_unlikely (GLRO (dl_debug_mask) & DL_DEBUG_FILES))
  ...
  if (__glibc_unlikely (
	  __mprotect ((void *) plt_aligned, pltsz, PROT_EXEC | PROT_READ) < 0))
...
x86_64_dynamic_after_reloc (struct link_map *map, int lazy)
{
  ...
  /* Ignore DT_X86_64_PLT if
	...
	value.  */
  if (map->l_info[DT_JMPREL] == NULL || map->l_info[DT_JMPREL]->d_un.d_ptr == 0
