...
#    define do_exchange_and_add_val_64_acq(pfx, mem, value)                   \
	...
	while ((__result = pfx##_compare_and_exchange_val_64_acq (            \
		    __memp, __result + __addval, __result))                   \
	       == __tmpval);                                                  \
	__result;                                                             \
      })
...
#    define do_add_val_64_acq(pfx, mem, value)                                \
	...
	while ((__oldval = pfx##_compare_and_exchange_val_64_acq (            \
		    __memp, __oldval + __addval, __oldval))                   \
	       == __tmpval);                                                  \
      }
#  endif
...
#  define atomic_exchange_and_add(mem, value) __sync_fetch_and_add (mem, value)
...
#  define atomic_increment(mem)                                               \
    __arch_increment_body (LOCK_PREFIX, __arch, mem)
...
#  define atomic_decrement(mem)                                               \
    __arch_decrement_body (LOCK_PREFIX, __arch, mem)
...
#  define atomic_bit_test_set(mem, bit)                                       \
    ({                                                                        \
      unsigned char __result;                                                 \
      if (sizeof (*mem) == 1)                                                 \
	else if (sizeof (*mem) == 2) else if (sizeof (*mem) == 4) else if (   \
	    __HAVE_64B_ATOMICS) else __atomic_link_error ();                  \
      __result;                                                               \
    })
...
#  define __arch_cprefix "cmpl $0, %%" SEG_REG ":%P3\n\tje 0f\n\tlock\n0:\t"
