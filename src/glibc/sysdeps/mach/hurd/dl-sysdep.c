...
rtld_hidden_data_def (__libc_stack_end)
    ...
    /* TODO: Initialize.  */
    void *_dl_random attribute_relro
    = NULL;
					       ...
					       ElfW (auxv_t) * auxv))
{
  void go (intptr_t *argdata)
  {
    ...
    GLRO (dl_platform)
	= NULL; /* Default to nothing known about the platform.  */
...
/* Minimal open/close/mmap/etc. implementation sufficient for initial loading
   of shared libraries.  These are weak definitions so that when the dynamic
   linker re-relocates itself to be user-visible (for -ldl), it will get the
   user's definition (i.e. usually libc's).
...
#  define check_no_hidden(name)                                               \
	...
	__attribute__ ((alias (#name))) __attribute_copy__ (name);
...
static error_t
open_file (const char *file_name, int flags, mach_port_t *port,
	   struct stat64 *stat)
{
  ...
  startdir = _dl_hurd_data->portarray[file_name[0] == '/' ? INIT_PORT_CRDIR
							  : INIT_PORT_CWDIR];

  while (file_name[0] == '/')
    file_name++;

  err = __dir_lookup (startdir, (char *) file_name, flags, 0, &doretry,
		      retryname, port);
...
weak_alias (__open, __open64) weak_alias (__open, __open_nocancel)
...
libc_hidden_weak (__pread64) weak_alias (__pread64, __pread64_nocancel)
...
libc_hidden_weak (__read) weak_alias (__read, __read_nocancel)
...
libc_hidden_weak (__write) weak_alias (__write, __write_nocancel)
...
#  endif
  ...
  err = __vm_map (__mach_task_self (), &mapaddr, (vm_size_t) len, mask,
		  !(flags & MAP_FIXED), memobj_rd, (vm_offset_t) offset,
		  flags & (MAP_COPY | MAP_PRIVATE), vmprot, VM_PROT_ALL,
...
int
__rtld_execve (const char *file_name, char *const argv[], char *const envp[])
{
  file_t file;
  error_t err;
  char *args, *env;
  ...
  please_dealloc
      = __alloca ((portarraysize + dtablesize) * sizeof (mach_port_t));
  pdp = please_dealloc;

  /* Get all ports that we may not know about and we should thus destroy.  */
  err = __mach_port_names (__mach_task_self (), &portnames, &nportnames,
...
#  endif
  ...
  err = __file_exec_paths (
      file, __mach_task_self (), flags, file_name,
      file_name[0] == '/' ? file_name : "", args, argslen, env, envlen, dtable,
      MACH_MSG_TYPE_COPY_SEND, dtablesize, ports, MACH_MSG_TYPE_COPY_SEND,
      portarraysize, intarray, INIT_INT_MAX, please_dealloc,
      pdp - please_dealloc, portnames, nportnames);
...
__getpid (void)
{
  ...
  if (__proc_getpids (_dl_hurd_data->portarray[INIT_PORT_PROC], &pid, &ppid,
		      &orphaned))
    return -1;
...
/* We need this alias to satisfy references from libc_pic.a objects
   that were affected by the libc_hidden_proto declaration for
   __strtoul_internal.  */
...
strong_alias (abort, __GI_abort) strong_alias (abort, __GI___fortify_fail)
    ...
    _hurd_intr_rpc_mach_msg (mach_msg_header_t *msg, mach_msg_option_t option,
			     mach_msg_size_t send_size,
			     mach_msg_size_t rcv_size, mach_port_t rcv_name,
			     mach_msg_timeout_t timeout, mach_port_t notify)
{
  return __mach_msg (msg, option, send_size, rcv_size, rcv_name, timeout,
		     notify);
}
