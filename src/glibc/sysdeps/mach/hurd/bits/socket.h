...
#    error                                                                     \
	"Never include <bits/socket.h> directly; use <sys/socket.h> instead."
#  endif
...
#  define __CMSG_PADDING(len)                                                 \
    ((sizeof (size_t) - ((len) & (sizeof (size_t) - 1)))                      \
     & (sizeof (size_t) - 1))
...
__NTH (__cmsg_nxthdr (struct msghdr *__mhdr, struct cmsghdr *__cmsg))
{
  ...
  size_t __size_needed
      = sizeof (struct cmsghdr) + __CMSG_PADDING (__cmsg->cmsg_len);
  ...
  if (((size_t) (__msg_control_ptr + __mhdr->msg_controllen - __cmsg_ptr)
       < __size_needed)
      || ((size_t) (__msg_control_ptr + __mhdr->msg_controllen - __cmsg_ptr
		    - __size_needed)
	  < __cmsg->cmsg_len))
