...
#define INTR_MSG_TRAP(msg, option, send_size, rcv_size, rcv_name, timeout,                              \
  ...
  ({                                                                                                    \
	...
	".globl _hurd_intr_rpc_msg_in_trap\n" /* Clear rax before we do the                             \
						 check for cancel below. This                                                                                                   \
						 is to detect rax being set                                                                                   \
						 to non-zero (actually                                  \
						 MACH_SEND_INTERRUPTED) from                                                                                                   \
						 the outside (namely,                                   \
						 _hurdsig_abort_rpcs), which                            \
						 signals us to skip the trap                                                                                       \
						 we were about to enter.  */                            \
	"				xor %[err], %[err]\n"                                                                      \
	"_hurd_intr_rpc_msg_about_to:" /* We need to make a last check of                               \
					  cancel, in case we got interrupted                            \
					  right before                                                  \
	...
	"_hurd_intr_rpc_msg_do:" /* Ok, prepare the mach_msg_trap arguments.                            \
				    We pass the first 6 args in registers,                                                                                          \
				    and push the last one and a fake return                             \
				    address.  */                                                        \
	"				pushq %[notify]\n"                                                                         \
	"				pushq $0\n"                                                                                \
	"_hurd_intr_rpc_msg_setup_done:" /* From here on, it is safe to make                            \
					    us jump over the syscall.  Now                              \
					    check if we have been told to                               \
					    skip the syscall while running                              \
					    the above.  */                                              \
	"				test %[err], %[err]\n"                                                                     \
	"				jnz _hurd_intr_rpc_msg_in_trap\n" /* Do the actual syscall.  */ \
	"				movq $-25, %%rax\n"                                                                        \
	"_hurd_intr_rpc_msg_do_trap:	syscall # status in %[err]\n"                                      \
	"_hurd_intr_rpc_msg_in_trap:" /* Clean the arguments.  */                                       \
	"				addq $16, %%rsp\n"                                                                         \
	"_hurd_intr_rpc_msg_sp_restored:"                                                               \
	: [err] "=&a"(err), "+S"(option), [intr_port] "=m"(*intr_port_p),                               \
	  "+r"(timeout_r9)                                                                              \
	: "D"(msg), "d"(send_size), "r"(rcv_size_r10),                                                  \
	  "r"(rcv_name_r8), [notify] "rm"((uintptr_t) notify),                                          \
	  [cancel] "m"(*cancel_p), [eintr] "i"(EINTR)                                                   \
	: "rcx", "r11");                                                                                \
    timeout = timeout_r9;                                                                               \
    err;                                                                                                \
  })
