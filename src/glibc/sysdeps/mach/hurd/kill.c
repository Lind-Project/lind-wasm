...
__kill (pid_t pid, int sig)
{
  ...
  void kill_pid (pid_t pid) /* Kill one PID.  */
  {
	...
	err = HURD_MSGPORT_RPC (
	    __proc_getmsgport (proc, pid, &msgport),
	    (taskerr = __proc_pid2task (proc, pid, &refport))
		? __proc_getsidport (proc, &refport)
		: 0,
	    1, kill_port (msgport, refport));
      }
    if (!err)
      delivered = 1;
  }
  ...
  if (pid <= 0)
      ...
      if (!err)
	{
	  ...
	  if (pids != pidbuf)
	    __vm_deallocate (__mach_task_self (), (vm_address_t) pids,
			     npids * sizeof (pids[0]));
	}
    }
  else
...
libc_hidden_def (__kill) weak_alias (__kill, kill)
