...
#    define __i386_set_gdt(thr, sel, desc)                                    \
      ((void) (thr), (void) (sel), (void) (desc), MIG_BAD_ID)
#  endif
...
#    define THREAD_SELF (*(tcbhead_t * __seg_gs *) offsetof (tcbhead_t, tcb))
#    define THREAD_GETMEM(descr, member)                                      \
      (*(__typeof (descr->member) __seg_gs *) offsetof (tcbhead_t, member))
#    define THREAD_GETMEM_NC(descr, member, idx)                              \
      (*(__typeof (descr->member[0])                                          \
	     __seg_gs *) (offsetof (tcbhead_t, member)                        \
			  + (idx) * sizeof (descr->member[0])))
...
#    define THREAD_GETMEM_NC(descr, member, idx)                              \
	...
	if (sizeof (__value) == 1)                                            \
	  ...
	  asm volatile ("movl %%gs:%P1(,%2,4),%0"                             \
			: "=r"(__value)                                       \
			: "i"(offsetof (tcbhead_t, member[0])), "r"(idx));    \
	else /* 8 */                                                          \
	  {                                                                   \
...
#    define THREAD_SETMEM(descr, member, value)                               \
	...
	if (sizeof (descr->member) == 1)                                      \
	  asm volatile ("movb %b0,%%gs:%P1"                                   \
			:                                                     \
			: "iq"(value), "i"(offsetof (tcbhead_t, member)));    \
	else if (sizeof (descr->member) == 4)                                 \
	  asm volatile ("movl %0,%%gs:%P1"                                    \
			:                                                     \
			: "ir"(value), "i"(offsetof (tcbhead_t, member)));    \
	else /* 8 */                                                          \
	  {                                                                   \
	...
	if (sizeof (descr->member[0]) == 1)                                   \
	  ...
	  {                                                                   \
	    asm volatile ("movl %%eax,%%gs:%P1(,%2,8)\n\t"                    \
			  ...
			  :                                                   \
			  : "A"((uint64_t) cast_to_integer (value)),          \
			    "i"(offsetof (tcbhead_t, member)), "r"(idx));     \
	  }                                                                   \
      })
...
#  define THREAD_COPY_STACK_GUARD(descr)                                      \
    ((descr)->stack_guard = THREAD_GETMEM (THREAD_SELF, stack_guard))

/* Set the pointer guard field in the TCB head.  */
#  define THREAD_SET_POINTER_GUARD(value)                                     \
    THREAD_SETMEM (THREAD_SELF, pointer_guard, value)
#  define THREAD_COPY_POINTER_GUARD(descr)                                    \
    ((descr)->pointer_guard = THREAD_GETMEM (THREAD_SELF, pointer_guard))
