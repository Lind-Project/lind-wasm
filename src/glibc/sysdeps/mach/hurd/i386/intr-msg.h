...
#define INTR_MSG_TRAP(msg, option, send_size, rcv_size, rcv_name, timeout,                              \
  ...
  ({                                                                                                    \
    ...
    asm (".globl _hurd_intr_rpc_msg_about_to\n"                                                         \
	 ...
	 ".globl _hurd_intr_rpc_msg_in_trap\n" /* Clear eax before we do the                            \
						  check for cancel below.                               \
						  This is to detect eax being                                                                                       \
						  set to non-zero (actually                             \
	 ...
	 "_hurd_intr_rpc_msg_do:" /* Ok, push the mach_msg_trap arguments and                           \
	 ...
	 "_hurd_intr_rpc_msg_in_trap:" /* Ok, clean the arguments and update                            \
					  OPTION and TIMEOUT.  */                                       \
	 "				addl $8, %%esp\n"                                                                         \
	 "				popl %1\n"                                                                                \
	 "				addl $12, %%esp\n"                                                                        \
...
#define SYSCALL_EXAMINE(state, callno)                                        \
  ({                                                                          \
    ...
    if (_hurdsig_catch_memory_fault (p))                                      \
      return 0;                                                               \
    if (result                                                                \
	= p->c[0] == 0x0000009a && (p->c[1] & 0x00ffffff) == 0x00000700)      \
