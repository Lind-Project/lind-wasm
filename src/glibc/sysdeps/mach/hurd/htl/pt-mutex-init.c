...
__pthread_mutex_init (pthread_mutex_t *mtxp, const pthread_mutexattr_t *attrp)
{
  ...
  mtxp->__flags
      = (attrp->__pshared == PTHREAD_PROCESS_SHARED ? GSYNC_SHARED : 0)
	| ((attrp->__prioceiling & PTHREAD_MUTEX_ROBUST) ? PTHREAD_MUTEX_ROBUST
							 : 0);

  mtxp->__type
      = (attrp->__mutex_type + (attrp->__mutex_type != __PTHREAD_MUTEX_TIMED));
