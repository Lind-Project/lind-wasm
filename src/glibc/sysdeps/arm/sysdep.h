...
.eabi_attribute 25,
	...
	/* The thumb2 encoding is reasonably complete.  Unless suppressed, use
	   it.  */
	.syntax unified
...
#  define LDST_INDEXED_NOINDEX(OP, R, T, X, Y)                                \
    add T, X, Y;                                                              \
    OP R, [T]
#  define LDST_INDEXED_INDEX(OP, R, X, Y) OP R, [ X, Y ]
...
#    ifdef __thumb2__
#      define LDST_PC_INDEXED(OP, R, T, X)                                    \
	LDST_INDEXED_NOINDEX (OP, R, T, pc, X)
#    else
