...
static ElfW (Addr) __dl_runtime_resolve (ElfW (Word), ElfW (Word), ElfW (Addr),
					 ElfW (Addr)) __attribute_used__;

static ElfW (Addr)
    __dl_runtime_resolve (ElfW (Word) sym_index, ElfW (Word) return_address,
			  ElfW (Addr) old_gpreg, ElfW (Addr) stub_pc)
{
	...
	default:
	  {
		...
		/* We need to keep the scope around so do some locking.  This
		   is not necessary for objects which cannot be unloaded or
		   when we are not using any threads (yet).  */
...
2:	" STRINGXP (SETUP_GP) "\n\
	...
	" STRINGXP (PTR_ADDIU) "	$29, " STRINGXP (
    ELF_DL_FRAME_SIZE) "\n\
	move	$25, $2\n\
	jr	$25\n\
...
_dl_runtime_pltresolve:\n\
	...
	.set noreorder\n\
	# Save arguments and sp value in stack.\n\
1:	" STRINGXP (PTR_SUBIU) "	$29, " STRINGXP (
    ELF_DL_PLT_FRAME_SIZE) "\n\
...
_dl_runtime_resolve:\n\
	.frame	$29, " STRINGXP (
    ELF_DL_FRAME_SIZE) ", $31\n\
	# Save arguments and sp value in stack.\n\t"
#  if _MIPS_ISA >= _MIPS_ISA_MIPS32
		       "save	" STRINGXP (
			   ELF_DL_FRAME_SIZE) ", $4-$7, $ra\n\t"
#  else
		       "addiu	$sp, -" STRINGXP (
			   ELF_DL_FRAME_SIZE) "\n\
	sw	$7, 32($sp)\n\
...
_dl_runtime_pltresolve:\n\
	.frame	$29, " STRINGXP (
    ELF_DL_PLT_FRAME_SIZE) ", $31\n\
	# Save arguments and sp value in stack.\n\t"
#  if _MIPS_ISA >= _MIPS_ISA_MIPS32
			   "save	" STRINGXP (
			       ELF_DL_PLT_FRAME_SIZE) ", $4-$7, $ra\n\t"
#  else
			   "addiu	$sp, -" STRINGXP (
			       ELF_DL_PLT_FRAME_SIZE) "\n\
	sw	$7, 40($sp)\n\
