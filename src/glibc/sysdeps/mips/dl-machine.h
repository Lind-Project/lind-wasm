...
static inline ElfW (Addr) * elf_mips_got_from_gpreg (ElfW (Addr) gpreg)
{
...
.Lcof2:	" STRINGXP (PTR_SUBU) " $8, $31, $8\n\
	...
	move $25, $17\n\
	jr $25\n\t" _RTLD_EPILOGUE (_dl_start_user) ".previous");
...
static inline ElfW (Addr)
    elf_machine_fixup_plt (struct link_map *map, lookup_t t,
			   ...
			   const ElfW (Rel) * reloc, ElfW (Addr) * reloc_addr,
			   ElfW (Addr) value)
{
	...
	if (symidx)
	  {
		...
		= (const ElfW (Word)) map->l_info[DT_MIPS (GOTSYM)]
		      ->d_un.d_val;
		...
		const ElfW (Word) local_gotno
		    = (const ElfW (Word)) map->l_info[DT_MIPS (LOCAL_GOTNO)]
			  ->d_un.d_val;
...
#  endif
  ...
  %s: Symbol `%s' has different size in shared object, consider re-linking\n",
	...
	memcpy (reloc_addr, (void *) value,
		sym->st_size < refsym->st_size ? sym->st_size
					       : refsym->st_size);
	break;
      }
...
elf_machine_rel (struct link_map *map, struct r_scope_elem *scope[],
		 ...
		 int skip_ifunc)
{
  elf_machine_reloc (map, scope, reloc->r_info, sym, version, reloc_addr, 0,
		     1);
}
...
/* Relocate GOT. */
static inline void __attribute__ ((always_inline))
elf_machine_got_rel (struct link_map *map, struct r_scope_elem *scope[],
		     int lazy)
{
  ElfW (Addr) * got;
  ElfW (Sym) * sym;
