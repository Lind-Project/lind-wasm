...
#define atomic_exchange_acq(mem, newvalue)                                    \
  ({                                                                          \
	...
	__asm __volatile ("1: cas2%.l %0:%R0,%1:%R1,(%2):(%3);"               \
			  "   jbne 1b"                                        \
			  : "=d"(__result)                                    \
			  : "d"((__typeof (*(mem))) (newvalue)), "r"(__memp), \
...
#define atomic_add(mem, value)                                                \
  (void) ({                                                                   \
    if (sizeof (*(mem)) == 1)                                                 \
      __asm __volatile ("add%.b %1,%0" : "+m"(*(mem)) : "id"(value));         \
    else if (sizeof (*(mem)) == 2)                                            \
      __asm __volatile ("add%.w %1,%0" : "+m"(*(mem)) : "id"(value));         \
    else if (sizeof (*(mem)) == 4)                                            \
      __asm __volatile ("add%.l %1,%0" : "+m"(*(mem)) : "id"(value));         \
    else                                                                      \
      {                                                                       \
	__typeof (mem) __memp = (mem);                                        \
...
#define atomic_increment_and_test(mem)                                        \
  ({                                                                          \
	...
	__asm __volatile ("1: move%.l %1,%2;"                                 \
			  ...
			  "   jbne 1b"                                        \
			  : "=&dm"(__result), "=d"(__oldval), "=&d"(__temp)   \
			  : "r"(__memp), "r"((char *) __memp + 4), "d"(0),    \
			    "1"(__oldval)                                     \
			  : "memory");                                        \
      }                                                                       \
    __result;                                                                 \
