...
__fesetenv (const fenv_t *envp)
{
  ...
  if (envp == FE_DFL_ENV)
    ...
    {
      temp.__control_register
	  |= (envp->__control_register & ((FE_ALL_EXCEPT << 6) | FE_UPWARD));
      temp.__status_register |= envp->__status_register & FE_ALL_EXCEPT;
    }

#ifdef __mcoldfire__
  __asm__ __volatile__ (
      "fmove%.l %0,%/fpiar" ::"dm"(temp.__instruction_address));
  __asm__ __volatile__ ("fmove%.l %0,%/fpcr" ::"dm"(temp.__control_register));
  __asm__ __volatile__ ("fmove%.l %0,%/fpsr" ::"dm"(temp.__status_register));
#else
