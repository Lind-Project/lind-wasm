...
static const _Float128 tiny = L (1.0e-4900), zero = 0.0,
		       pi_o_4
		       = L (7.85398163397448309615660845819875699e-01), /* 3ffe921fb54442d18469898cc51701b8
									 */
...
__ieee754_atan2l (_Float128 y, _Float128 x)
{
  ...
  if (((ix | ((lx | -lx) >> 63)) > 0x7fff000000000000LL)
      || ((iy | ((ly | -ly) >> 63))
	  > 0x7fff000000000000LL)) /* x or y is NaN */
    return x + y;
  if (((hx - 0x3fff000000000000LL) | lx) == 0)
    return __atanl (y);			   /* x=1.0L */
      ...
      if (iy == 0x7fff000000000000LL)
	{
	  switch (m)
	    ...
	    case 1:
	      return -pi_o_4 - tiny; /* atan(-INF,+INF) */
	    case 2:
	      return 3 * pi_o_4 + tiny; /*atan(+INF,-INF)*/
	    case 3:
	      return -3 * pi_o_4 - tiny; /*atan(-INF,-INF)*/
	    }
	}
  ...
  k = (iy - ix) >> 48;
  if (k > 120)
    z = pi_o_2 + L (0.5) * pi_lo; /* |y/x| >  2**120 */
  else if (hx < 0 && k < -120)
    z = 0; /* |y|/x < -2**120 */
  else
    z = __atanl (fabsl (y / x)); /* safe to do y/x */
  switch (m)
    {
    case 0:
      return z; /* atan(+,+) */
