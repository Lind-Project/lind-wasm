...
mp_size_t
__mpn_extract_double (mp_ptr res_ptr, mp_size_t size, int *expt, int *is_neg,
		      double value)
{
...
#define NUM_LEADING_ZEROS                                                     \
  (BITS_PER_MP_LIMB - (DBL_MANT_DIG - ((N - 1) * BITS_PER_MP_LIMB)))
  ...
  if (u.ieee.exponent == 0)
	...
	*expt = 0;
	  ...
	  if (res_ptr[N - 1] != 0)
	    {
	      ...
	      res_ptr[N - 1]
		  = res_ptr[1] << cnt
		    | (N - 1) * (res_ptr[0] >> (BITS_PER_MP_LIMB - cnt));
	      res_ptr[0] <<= cnt;
#else
	      res_ptr[N - 1] <<= cnt;
#endif
    ...
    res_ptr[N - 1] |= (mp_limb_t) 1
		      << (DBL_MANT_DIG - 1 - ((N - 1) * BITS_PER_MP_LIMB));
