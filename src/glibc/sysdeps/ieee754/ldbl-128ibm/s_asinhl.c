...
static const long double one
    = 1.00000000000000000000e+00L,		 /* 0x3ff0000000000000, 0 */
    ln2 = 0.6931471805599453094172321214581766L, /* 0x3fe62e42fefa39ef,
						    0x3c7abc9e3b398040 */
...
__asinhl (long double x)
{
  ...
  if (ix < 0x3c70000000000000LL)
    { /* |x|<2**-56 */
      math_check_force_underflow (x);
      if (huge + x > one)
	return x; /* return x inexact except 0 */
    }
  if (ix > 0x4370000000000000LL)
    { /* |x| > 2**56 */
      w = __ieee754_logl (fabsl (x)) + ln2;
    }
  else if (ix > 0x4000000000000000LL)
    { /* 2**56 >= |x| > 2.0 */
      t = fabs (x);
