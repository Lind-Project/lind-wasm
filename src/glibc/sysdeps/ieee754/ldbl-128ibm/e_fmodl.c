...
static const long double one = 1.0, Zero[] = {
  0.0,
  -0.0,
};
...
__ieee754_fmodl (long double x, long double y)
{
  ...
  if (__builtin_expect (hy == 0 || (hx >= 0x7ff0000000000000LL)
			    || /* y=0,or x not finite */
			    (hy > 0x7ff0000000000000LL),
			0)) /* or y is NaN */
    return (x * y) / (x * y);
  if (__glibc_unlikely (hx <= hy))
	...
	return x;
	 ...
	 x and y must be equal.  */
      if ((lx & 0x7fffffffffffffffLL) == 0 && (ly & 0x7fffffffffffffffLL) == 0)
