...
struct __libc {
	...
	size_t page_size;
#endif
	struct __locale_struct_wasi global_locale;
#if defined(__wasilibc_unmodified_upstream) || defined(_REENTRANT)
#else
	struct __locale_struct_wasi *current_locale;
#endif
};
...
{
	struct pthread *td = p;
	// TODO: check what other attribute in pthread struct need to be initialized here
#ifdef __wasilibc_unmodified_upstream
	...
	__default_stacksize =
		...
		bounds.size : DEFAULT_STACK_MAX;
	td->stackblock = bounds.base;
	td->stackblock_size = bounds.size;
	td->guardsize = 0;
	/*
