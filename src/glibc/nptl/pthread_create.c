...
out:
  ...
  pd->tid = 0;
  MAKE_LEGACY_SYSCALL(FUTEX_SYSCALL, "syscall|futex", (uint64_t) TRANSLATE_GUEST_POINTER_TO_HOST(&pd->tid), (uint64_t) FUTEX_WAKE, (uint64_t) 1, (uint64_t)0, 0, (uint64_t)0, WRAPPED_SYSCALL);
  while (1)
