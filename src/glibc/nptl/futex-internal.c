                                ...
                                int private, bool cancel)
{
    ...
    return MAKE_TRADITION(FUTEX_SYSCALL, "syscall|futex", host_futex_word, (uint64_t) op, (uint64_t) expected, host_timeout, 0, (uint64_t)FUTEX_BITSET_MATCH_ANY, RAW_SYSCALL);
}
                                ...
                                int private, bool cancel)
{
    ...
    return MAKE_TRADITION(FUTEX_SYSCALL, "syscall|futex", host_futex_word, (uint64_t) op, (uint64_t) expected, host_abstime, 0, (uint64_t)FUTEX_BITSET_MATCH_ANY, RAW_SYSCALL);
}
		   ...
		   const struct __timespec64 *abstime, int private)
{
  ...
  uint64_t host_abstime = TRANSLATE_GUEST_POINTER_TO_HOST(abstime);
  err = MAKE_TRADITION(FUTEX_SYSCALL, "syscall|futex", host_futex_word, (uint64_t) op_pi2, (uint64_t) 0, host_abstime, 0, (uint64_t)0, RAW_SYSCALL);
#else
...
# ifdef __ASSUME_TIME64_SYSCALLS
  uint64_t host_abstime = TRANSLATE_GUEST_POINTER_TO_HOST(abstime);
  err = MAKE_TRADITION(FUTEX_SYSCALL, "syscall|futex", host_futex_word, (uint64_t) op_pi, (uint64_t) 0, host_abstime, 0, (uint64_t)0, RAW_SYSCALL);
# else
  bool need_time64 = abstime != NULL && !in_int32_t_range (abstime->tv_sec);
  if (need_time64) 
    {
      ...
      if (abstime != NULL)
	{
      ...
      uint64_t host_timeout = TRANSLATE_GUEST_POINTER_TO_HOST(pts32);
      err = MAKE_TRADITION(FUTEX_SYSCALL, "syscall|futex", host_futex_word, (uint64_t) op_pi, (uint64_t) 0, host_timeout, 0, (uint64_t)0, RAW_SYSCALL);
    }
# endif	 /* __ASSUME_TIME64_SYSCALLS */
