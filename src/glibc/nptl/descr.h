...
#    define ENQUEUE_MUTEX_BOTH(mutex, val)                                    \
      do                                                                      \
	{                                                                     \
	  __pthread_list_t *next                                              \
	      = (__pthread_list_t *) ((((uintptr_t) THREAD_GETMEM (           \
					   THREAD_SELF, robust_head.list))    \
				       & ~1ul)                                \
				      - QUEUE_PTR_ADJUST);                    \
	  next->__prev = (void *) &mutex->__data.__list.__next;               \
	  mutex->__data.__list.__next                                         \
	      = THREAD_GETMEM (THREAD_SELF, robust_head.list);                \
	  mutex->__data.__list.__prev = (void *) &THREAD_SELF->robust_head;   \
	  /* Ensure that the new list entry is ready before we insert it.  */ \
	  __asm ("" ::: "memory");                                            \
	  THREAD_SETMEM (                                                     \
	      THREAD_SELF, robust_head.list,                                  \
	      (void *) (((uintptr_t) &mutex->__data.__list.__next) | val));   \
	}                                                                     \
      while (0)
#    define DEQUEUE_MUTEX(mutex)                                              \
	...
	{                                                                     \
	  __pthread_list_t *next                                              \
	      = (__pthread_list_t *) ((char *) (((uintptr_t) mutex->__data    \
						     .__list.__next)          \
						& ~1ul)                       \
				      - QUEUE_PTR_ADJUST);                    \
	  next->__prev = mutex->__data.__list.__prev;                         \
	  __pthread_list_t *prev                                              \
	      = (__pthread_list_t *) ((char *) (((uintptr_t) mutex->__data    \
						     .__list.__prev)          \
						& ~1ul)                       \
				      - QUEUE_PTR_ADJUST);                    \
	  prev->__next = mutex->__data.__list.__next;                         \
	  /* Ensure that we remove the entry from the list before we change   \
	     the                                                              \
...
#    define ENQUEUE_MUTEX_BOTH(mutex, val)                                    \
	...
	{                                                                     \
	  ...
	  __asm ("" ::: "memory");                                            \
	  THREAD_SETMEM (                                                     \
	      THREAD_SELF, robust_list.__next,                                \
...
#    define DEQUEUE_MUTEX(mutex)                                              \
      do                                                                      \
	{                                                                     \
	  __pthread_slist_t *runp                                             \
	      = (__pthread_slist_t *) (((uintptr_t) THREAD_GETMEM (           \
					   THREAD_SELF, robust_list.__next))  \
				       & ~1ul);                               \
	  if (runp == &mutex->__data.__list)                                  \
	    ...
	    {                                                                 \
	      __pthread_slist_t *next                                         \
		  = (__pthread_slist_t *) (((uintptr_t) runp->__next)         \
					   & ~1ul);                           \
	      while (next != &mutex->__data.__list)                           \
		{                                                             \
		  runp = next;                                                \
		  next = (__pthread_slist_t *) (((uintptr_t) runp->__next)    \
						& ~1ul);                      \
		}                                                             \
                                                                              \
	      runp->__next = next->__next;                                    \
	      /* Ensure that we remove the entry from the list before we      \
		 change the                                                   \
		 __next pointer of the entry, which is read by the kernel. */                                                                           \
	      __asm ("" ::: "memory");                                        \
	      mutex->__data.__list.__next = NULL;                             \
...
{
  return (value
	  & (CANCELSTATE_BITMASK | CANCELED_BITMASK | EXITING_BITMASK
	     | TERMINATED_BITMASK))
	 == CANCELED_BITMASK;
}
