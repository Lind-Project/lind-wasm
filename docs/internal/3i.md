...
In a traditional Linux environment, extending or intercepting system calls (for example, adding a new filesystem, tracing, or filtering calls) requires kernel modifications or mechanisms such as `ptrace`, which incur heavy overhead and depend on kernel-level mediation.
...
Consider the `fork` system call: if a cage invokes `fork` and the grate simply executes a native `fork`, the grate — not the originating cage — would be duplicated. To prevent this, each `make_syscall` invocation explicitly specifies the target cage whose state and identity the system call should operate on.
