...
One final important feature of · is the ability for a grate to perform a system call as though another cage had issued it. Consider the fork system call: if a cage invokes fork and the grate simply executes a native fork, the grate -- not the originating cage -- would be duplicated. To prevent this, each `make_syscall` invocation explicitly specifies the target cage whose state and identity the system call should operate on.
...
Considering the two scenarios mentioned above, we need to store the runtime context of each active cage/grate in order to re-enter and execute code logic of the Wasm module in grate calls. 

Rust’s lifetime rules prevent us from storing a `Store` or any of its borrowed references in a static global table: a `Store` is parameterized by a non-`'static` lifetime that reflects the fact that all runtime state inside Wasmtime is owned by the caller, not by the engine. As a result, 3i cannot persistently retain a `Store` or `Instance` reference for later re-entry.

To work around this restriction, lind-wasm stores a raw pointer to the target module’s `VMContext`, which does have `'static` lifetime from Rust’s perspective. The `VMContext` contains enough structural information (e.g., memory tables, runtime limits, etc.) for 3i to reconstruct the corresponding `Store` and `Instance` on demand. During a grate callback, 3i retrieves the `VMContext` pointer, rebuilds the appropriate execution environment for the `(cageid, threadid)` pair, and then re-enters the Wasm module through a registered re-entry trampoline. This approach enables safe cross-instance re-entry without violating Rust’s ownership model while still preserving Wasmtime’s isolation semantics.

During module initialization, Wasmtime obtains a raw pointer to each instance’s low-level VMContext. However, the registration of interposition handlers (`register_handler`) typically occurs after initialization. To bridge this timing gap, Wasmtime maintains a runtime-local table:
...
The purpose of these data structures is to provide a safe and stable mechanism for storing and using both the callback entrypoint (`fn_ptr`) and the Wasmtime execution context (`VMContext`) across the boundary between Wasmtime and 3i.
...
Wasmtime gets the pointer which points to the instance’s `VMContext` and stores both the callback and context pointers in the `GRATE_FN_WASM_TABLE`.
...
2. 3i retrieves the stored function pointer and `VMContext`.
