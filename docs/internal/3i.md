...
3i (Three-I) provides a general means for [cages](doc/index.md#cage) (and [grates](doc/index.md#grates)) to make system calls and intercept system calls via a programmable system call table.  The goal is to enable complex functionality without modifying the [microvisor](doc/index.md#core-concepts) or increasing the microvisor's trusted computing base. 
...
Lind-Wasm maintains a global, per-cage pool of `VMContext` pointers, indexed by `cage_id`. Each cage owns a FIFO queue that stores the execution contexts currently available to it. The total number of cages is fixed at startup (`MAX_CAGEID`).
