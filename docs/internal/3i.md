...
Each Wasm module in Wasmtime has its own `VMContext`, which encapsulates all runtime state required for that instance’s execution. When 3i performs a cross-module call (e.g., from _cage A_ to a _grate B_), the currently executing context belongs to _A_, but 3i needs to load the latest runtime state of _B_ before re-entering its Wasm code. 

One aspect of Wasm that causes challenges is the fact that each thread in a Wasm runtime needs its own Wasm module, since this is what contains its stack.  As a result, you can run into a situation where a grate is called by a cage, but there is no thread stack (i.e. Wasm module) available.  

Further complicating this issue, sometimes it is necessary to know which thread stack (i.e. Wasm module) has made a call when you are further down the call stack.  This is because a call like fork needs to run only that thread in the newly forked process.  
