...
Another aspect of Wasm that causes challenges is the fact that each thread in a Wasm runtime needs its own Wasm module, since this is what contains its stack. As a result, you can run into a situation where a grate is called by a cage, but there is no thread stack (i.e. Wasm module) available. Further complicating this issue, sometimes it is necessary to know which thread stack (i.e. Wasm module) has made a call when you are further down the call stack.  This is because a call like fork needs to run only that thread in the newly forked process.  
...
When the user’s Wasm code calls `register_handler`, the entry is extracted and passed to 3i, where it becomes the canonical reference for that `(cageid, threadid)` pair.
