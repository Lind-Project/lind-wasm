Start from commit (commit 337a15a) of libcpp-alice branch
...
Where we start with
Alice's issue #245 describes a slightly outdated solution to the above problem, Alice later started a new branch (libcpp-alice) and basically formalized that solution. This documentation by Ren will attempt to explain her solution (grossly) and document what is exactly done to achieve the overarching goal. 
Existing problem
...
This error is also recorded in issue #740 by Nick, as mentioned above.
...
```
 Then you will quickly see the error described in issue #740.
...
```
The above step is also mentioned in issue #245; and it will create llvm-project/ dir for us, resolving one of the two missing dependencies issue. Again, specifically ver 18 is used. (contrary to the issue #245 which claims ver 16)
...
and somewhere near line 456 (I don't think it is exactly that line, but a few dozen lines above it) you should see the definition of convert_to_timespec() function, its return statement should be modified by adding reinterpret_cast<long*>  and you should end up with something like this:
...
And you should now have two files:

trial/hello.cpp.cwasm and trial/hello.cpp.wasm

where, it seems that, the cwasm ELF file is not an intended product. I should look into it.

And that is the entire workflow to get .wasm binary compiled!

–TODO for ren: document the exception error and explain overall why things are done the way they are under issue #795
...
start from main, copy over all changes from alice-libcpp branch, and automate my manual steps, then submit 2 prs: one for documentation in .md, another one for the code itself.
