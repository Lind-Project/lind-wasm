// DESCRIPTION: (AF_UNIX, STREAM|DGRAM) RTT through ping-pong of PARAM bytes.
#include "bench.h"
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
...
void uds_dgram(int msg_size) {
	...
	char param[30] = "";
	sprintf(param, "(DGRAM, %d)", msg_size);
	emit_result_string("Unix Domain Socket (RTT)", param,
...
void uds_stream(int msg_size) {
	...
	char param[30] = "";
	sprintf(param, "(STREAM, %d)", msg_size);
	emit_result_string("Unix Domain Socket (RTT)", param,
			   (end - start) / loops, loops);
}
