 ...
 *
 * CURRENTLY SKIPPED (skip_test_cases.txt), and it MUST stay skipped until
 * the prlimit64 panic is fixed. On the current runtime the setrlimit()
 * below reaches prlimit64_syscall's lind_debug_panic!, and with
 * lind-logging on by default and PanicBehavior::PanicAndExit that aborts
 * the whole runtime and every cage in it -- this test does not fail, it
 * takes the harness down with it. Beyond that it needs, each tracked in
 * its own issue:
 *   - __setrlimit is not wired to rawposix and silently does nothing.
 *   - the per-cage memory quota (--max-cage-memory) this test bounds
 *     itself with; without it there is nothing for setrlimit to lower.
 * Remove the skip_test_cases.txt entry once all three have landed.
 *
