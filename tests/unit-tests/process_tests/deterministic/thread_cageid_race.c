/* Test: pthread_create must not clobber __grateos_cageid in shared memory.
 *
 * Before the clone3.c fix, the child thread's clone3 return path would
 * write __grateos_cageid = 0 to shared linear memory before re-querying
 * the host.  This raced with the parent (and sibling) threads, which
 * read __grateos_cageid during every syscall.  A zero cage ID causes a
