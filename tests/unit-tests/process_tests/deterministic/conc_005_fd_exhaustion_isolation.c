 ...
 * EMFILE while exhausted, not EBADF. dup2() onto an already-open
 * descriptor must still SUCCEED while exhausted, since it reuses a slot
 * rather than allocating one.
 *
 * CURRENTLY SKIPPED (skip_test_cases.txt). This does not pass on the
 * current runtime and is waiting on three fixes, each tracked in its own
 * issue:
 *   - open()/openat() leak a host fd on every EMFILE. Beyond failing,
 *     running this unfixed burns host descriptors that no cage can
 *     reclaim, which degrades the whole machine, not just this test.
 *   - fcntl(F_DUPFD)/F_DUPFD_CLOEXEC report EBADF where POSIX wants
 *     EMFILE -- what assertion (A3) below checks.
 *   - dup_syscall does not check libc::dup() for failure.
 * Remove the skip_test_cases.txt entry when all three have landed.
 *
...
static void child_a(barrier_t *b)
{
    ...
    if (hit_limit) {
        ...
        /* (A3) F_DUPFD/F_DUPFD_CLOEXEC currently report EBADF, which
