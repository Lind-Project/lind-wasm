 ...
 *
 * CURRENTLY SKIPPED (skip_test_cases.txt), on one assertion only: the
 * dup2_edge_cases() check that dup2(BADFD, BADFD) fails with EBADF.
 * dup2_syscall validates oldfd only AFTER its oldfd == newfd fast path, so
 * that call currently reports success and hands back a descriptor that was
 * never open. Everything else in this file passes today. Remove the
 * skip_test_cases.txt entry when that fix lands.
 *
...
/*                                                                     */
/* This is the one phase that fails on the current runtime, and the    */
/* sole reason this file is in skip_test_cases.txt.                     */
/* ------------------------------------------------------------------ */
