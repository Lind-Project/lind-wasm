...
// Dispatcher function
long long pass_fptr_to_wt(uint64_t fn_ptr_uint, uint64_t cageid, uint64_t arg1,
                    uint64_t arg1cage, uint64_t arg2, uint64_t arg2cage,
                    uint64_t arg3, uint64_t arg3cage, uint64_t arg4,
                    uint64_t arg4cage, uint64_t arg5, uint64_t arg5cage,
                    uint64_t arg6, uint64_t arg6cage) {
  if (fn_ptr_uint == 0) {
    fprintf(stderr, "[Grate|interpose-exit] Invalid function ptr\n");
    assert(0);
  }

  printf("[Grate|interpose-exit] Handling function ptr: %llu from cage: %llu\n",
         fn_ptr_uint, cageid);
...
int exit_grate(uint64_t cageid, 
    ...
    uint64_t arg6, uint64_t arg6cage) {
  printf("[Grate|interpose-exit] In exit_grate %d handler for cage: %llu\n",
         getpid(), cageid);
...
int main(int argc, char *argv[]) {
    ...
    if (pid < 0) {
        ...
        int cageid = getpid();
        // Set the exit (syscallnum=60) of this cage to call this grate
        // function exit_grate 
        // Syntax of register_handler:
        // <targetcage, targetcallnum, this_grate_id, fn_ptr_u64)>
        uint64_t fn_ptr_addr = (uint64_t)(uintptr_t)&exit_grate;
        printf("[Grate|interpose-exit] Registering exit handler for cage %d in "
                "grate %d with fn ptr addr: %llu\n",
                cageid, grateid, fn_ptr_addr);
        int ret = register_handler(cageid, 60, grateid, fn_ptr_addr);
        if (ret != 0) {
            fprintf(stderr, "[Grate|interpose-exit] Failed to register handler for cage %d in "
    ...
    while (wait(&status) > 0) {
        if (status != 0) {
        fprintf(stderr, "[Grate|interpose-exit] FAIL: child exited with status %d\n", status);
        assert(0);
        }
    }

    printf("[Grate|interpose-exit] PASS\n");
    return 0;
}
